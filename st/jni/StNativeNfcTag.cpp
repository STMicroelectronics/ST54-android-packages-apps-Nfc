...
void nativeNfcTag_cacheNonNciCardDetection()
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << 
        StringPrintf("%s: cardDetectTimeout = %d", __func__,cardDetectTimeout);
    sNonNciCardDetectionTimer.set(cardDetectTimeout, nonNciCardTimerProc);
    DLOG_IF(INFO, nfc_debug_enabled) << 
        StringPrintf("%s: sNonNciCard_t.sProtocolType=0x%x sNonNciCard_t.srfInterfaceType =0x%x ",
...
void nativeNfcTag_handleNonNciCardDetection(tNFA_CONN_EVT_DATA* eventData)
{
    ...
    if((eventData->activated.activate_ntf.intf_param.type == sNonNciCard_t.srfInterfaceType)  &&
    ( eventData->activated.activate_ntf.protocol == sNonNciCard_t.sProtocolType))
    {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << 
        StringPrintf("%s: eventData->activated.activate_ntf.protocol =0x%x eventData->activated.activate_ntf.intf_param.type =0x%x", 
        __func__,eventData->activated.activate_ntf.protocol, eventData->activated.activate_ntf.intf_param.type);
}
...
tNFA_STATUS nativeNfcTag_sendDeselectCommandIfNeeded(){
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter - sCurrentRfInterface: 0x%02X,  "
  ...
  if ((sCurrentRfInterface == NFA_INTERFACE_FRAME)
      && (NFC_GetNCIVersion() >= NCI_VERSION_2_0)) {
    {
      ...
      if (sCurrentActivatedProtocl == NFA_PROTOCOL_T2T) {
        status = NFA_SendRawFrame(RW_TAG_SLP_REQ, sizeof(RW_TAG_SLP_REQ),
            0);
...
void nativeNfcTag_doSelectTag() {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: sCurrentConnectedHandle = %d",
          __func__, sCurrentConnectedHandle);
...
static int reSelect(tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                    "%s: enter; rf intf = %d, current intf = %d", __func__,
    ...
    do {
        ...
        if (natTag.isNdefDetectionTimedOut()) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                            "%s: ndef detection timeout; break", __func__);
...

        {
            ...
            if (natTag.isCashBeeActivated() == true || natTag
                            .isEzLinkTagActivated()
                                                       == true
                || sNonNciCard_t.chinaTransp_Card == true) {
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                "%s: deactivate to sleep", __func__);
            ...
            if (sReconnectEvent.wait(wasStopped ? 100:1000) == false) // if timeout occurred
            {
                ...
                if(wasStopped){
                  ...
                  if (NFA_STATUS_OK != (status = NFA_StartRfDiscovery())) {
                      LOG(ERROR) << StringPrintf(
                          "%s: NFA_StartRfDiscovery failed, status = %d",
                          __func__, status);
                  }
                }
                break;
        ...
        if (natTag.getActivationState() == NfcTag::Idle) {
            ...
            if (natTag.mActivationParams_t.mTechLibNfcTypes == NFC_PROTOCOL_ISO_DEP) {
                if (natTag.mActivationParams_t.mTechParams == NFC_DISCOVERY_TYPE_POLL_A) {
                    ...
                    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                    "%s: mEzLinkTypeTag detected", __func__);
        ...
        if (!(natTag.isCashBeeActivated() == true || natTag.isEzLinkTagActivated()
                        == true
              || sNonNciCard_t.chinaTransp_Card == true)) {
...

        {
            ...
            if (natTag.isCashBeeActivated() == true || natTag
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                "%s: select interface %u", __func__,
                                rfInterface);
                if (NFA_STATUS_OK != (status = NFA_Select(
                                natTag.mTechHandles[handle],
                                natTag.mTechLibNfcTypes[handle], rfInterface))) {
                    LOG(ERROR) << StringPrintf(
                                    "%s: NFA_Select failed, status = %d -- fallback deactivate to idle",
                                    __func__, status);
            ...
            if (sReconnectEvent.wait(1000) == false) // if timeout occured
            {
                ...
                if (!(natTag.isCashBeeActivated() == true || natTag
                      ...
                      || sNonNciCard_t.chinaTransp_Card == true)) {
                    status = NFA_Deactivate(FALSE);
                    if (status != NFA_STATUS_OK) LOG(ERROR)
                                    << StringPrintf("%s: deactivate failed; error=0x%X",
                                                    __func__, status);
                }
                break;
            }
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                        "%s: select completed; sConnectOk=%d", __func__,
                        sConnectOk);
        if (NfcTag::getInstance().getActivationState() != NfcTag::Active) {
            LOG(ERROR) << StringPrintf("%s: tag is not active", __func__);
            rVal = STATUS_CODE_TARGET_LOST;
            if ((!retry_cnt) || (natTag.mTechLibNfcTypes[handle] != NFC_PROTOCOL_ISO_DEP))break;
        }
...
//            sCurrentRfInterface = rfInterface;
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit; status=%d",
                                                     __func__, rVal);
    return rVal;
}
...
static bool switchRfInterface(tNFA_INTF_TYPE rfInterface) {
  ...
  if ((sCurrentConnectedTargetProtocol != NFC_PROTOCOL_ISO_DEP) &&
      (sCurrentConnectedTargetProtocol != NFC_PROTOCOL_MIFARE)) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: protocol: %d not ISO_DEP nor MFC, do nothing", __func__,
...
static jint nativeNfcTag_doCheckNdef(JNIEnv* e, jobject o, jintArray ndefInfo) {
  ...
  if(sIsP2pPrioLogic){
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: P2P prio logic active, do not check NDEF", __func__);
    ...
    if (NFCSTATUS_SUCCESS != nativeNfcTag_doReconnect(e, o)) {
      LOG(ERROR) << StringPrintf(
          "%s: Reconnect failed so return error", __func__);
      sIsCheckingNDef = false;
      return NFA_STATUS_FAILED;
    }
  }
...
//              if((sCurrentConnectedTargetProtocol == NFC_PROTOCOL_ISO_DEP) && (sIsoDepPresCheckCnt == 1)){
//                  sIsoDepPresCheckCnt = 0;
//              }
          }
...
//              DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
//                  "%s(%d): pres check ISO_DEP NAK failed, try 3 more times (attempt #%d)",
