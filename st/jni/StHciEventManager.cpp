...
**
*******************************************************************************/
void StHciEventManager::notifyTransactionListenersOfAid(std::vector<uint8_t> aid,
...
void StHciEventManager::nfaHciCallback(tNFA_HCI_EVT event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s - event=%d code=%d pipe=%d len=%d", fn, event, eventData->rcvd_evt.evt_code,
...
//  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
//          "%s: EVT_TRANSACTION rx; pipe = 0x%02X, Host HCI ID = 0x%02X, Host NFCEE ID = 0x%02X",
//          fn, pipeId, hostId, nfceeId);
