...
void NfcStExtensions::initialize(nfc_jni_native_data* native) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: tag polling tech mask=0x%lX", fn, num);
...
//  if (num != 0) {
    ...
    /* NCI 2.0 - End */
    mRfConfig.techArray[CE_IDX] = mHostListenTechMask & HCE_TECH_MASK; //Allow tech A/B
//  }
...
//  }
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: mRfConfig.techArray[READER_IDX] = 0x%X", fn,
          mRfConfig.techArray[READER_IDX]);
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: mRfConfig.techArray[CE_IDX] = 0x%X", fn,
          mRfConfig.techArray[CE_IDX]);
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: mRfConfig.techArray[P2P_LISTEN_IDX] = 0x%X", fn,
          mRfConfig.techArray[P2P_LISTEN_IDX]);
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: mRfConfig.techArray[P2P_POLL_IDX] = 0x%X", fn,
...
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Core stack trace level=0x%X", fn, num);
//    } else {
//        num = 0;
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Core stack trace level could not read=0x%X", fn, num);
//    }
//
//    if (num) {
//        mTraceConfig.coreStack = num;
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Stored Core stack trace level: %lu", fn, mTraceConfig.coreStack);
//    }
//
//    if (GetNumValue("PROTOCOL_TRACE_LEVEL", &num, sizeof(num))) {
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: HAL trace level=0x%X", fn, num);
//    } else {
//        num = 0;
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: HAL trace level could not read=0x%X", fn, num);
//    }
//
//    if (num) {
//        mTraceConfig.hal = num;
//        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Stored HAL trace level: %lu", fn, mTraceConfig.hal);
//    }
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Selected Screen Off state 0x%X", fn,
  ...
  //  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
  //          "%s: user selected Switch Off state 0x%X", fn, mCeOnSwitchOffState);
  //}
  updateSwitchOffMode();
...
static const char* getHostName(int host_id) {
  ...
  switch (host_id) {
    ...
    case 2:
      ...
      break;
    default:
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Invalid host id!!!",
                                                      fn);
      break;
  }
...
/* NCI 2.0 - Begin */bool NfcStExtensions::getPipesInfo() {
  ...
  for (idx = 1; idx < 3; idx++) {
    ...
    if (StSecureElement::getInstance().isSEConnected((idx == 1 ? 0x02 : 0xc0)) == true) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Requesting info for host %s", fn, host);
      ...
      if (idx == 1) {
        ...
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                "%s: Active UICC is at slot 0x%02X", fn, nfceeId);
        ...
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                "%s: Active SE is at slot 0x%02X", fn, nfceeId);
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < mPipesInfo[i + 1].nb_pipes; j++) {
      if (mPipesInfo[i + 1].data[j].dest_host == 0x01) {
        ...
        mPipesInfo[0].data[dhIdx].dest_gate = mPipesInfo[i + 1].data[j]
                .dest_gate;
        mPipesInfo[0].data[dhIdx].dest_host = mPipesInfo[i + 1].data[j]
                .dest_host;
        mPipesInfo[0].data[dhIdx].pipe_id = mPipesInfo[i + 1].data[j].pipe_id;
        mPipesInfo[0].data[dhIdx].pipe_state = mPipesInfo[i + 1].data[j]
                .pipe_state;
        mPipesInfo[0].data[dhIdx].source_gate = mPipesInfo[i + 1].data[j]
                .source_gate;
        mPipesInfo[0].data[dhIdx].source_host = mPipesInfo[i + 1].data[j]
                .source_host;
  ...
  for (i = 0; i < 3; i++) {
    host = getHostName(i);
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Found %d pipes for %s",
                                                    fn, mPipesInfo[i].nb_pipes,
                                                    host);
    for (j = 0; j < mPipesInfo[i].nb_pipes; j++) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Info for pipe 0x%x: source host is 0x%x, destination host is 0x%x, gate is 0x%x, state is 0x%x",
              fn, mPipesInfo[i].data[j].pipe_id,
              mPipesInfo[i].data[j].source_host,
              mPipesInfo[i].data[j].dest_host,
              mPipesInfo[i].data[j].source_gate,
...
void NfcStExtensions::nfaVsCbActionRequest(uint8_t oid, uint16_t len,
  ...
  if (sStExtensions.mIsWaitingEvent.pipeInfo == true) { //Pipes Info
    if (len == 0) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: PROP_CMD_RSP; No data returned !!!!", fn);
    } else {
      int i = 0, nb_pipes = 0;
    ...
    if (p_msg[3] == NFA_STATUS_OK) {
      ...
      if (sStExtensions.mPropTestRspLen) {
        sStExtensions.mPropTestRspPtr = (uint8_t*) GKI_os_malloc(
...
uint8_t NfcStExtensions::getPipeState(uint8_t pipe_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Pipe not found in stored data - Not created", fn);
  return 0;
}
...
uint8_t NfcStExtensions::getPipeIdForGate(uint8_t host_id, uint8_t gate_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Requesting pipe id for gate 0x%x on host 0x%x", fn, gate_id,
          host_id);
  /* NCI 2.0 - Begin */
  int i, idx = 0;
  /* NCI 2.0 - End */
  ...
  for (i = 0; i < mPipesInfo[idx].nb_pipes; i++) {
    if (mPipesInfo[idx].data[i].source_gate == gate_id) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Pipe 0x%x belongs to gate 0x%x", fn,
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Gate not found in stored data - No pipe created on that gate",
          fn);
...
uint8_t NfcStExtensions::getHostIdForPipe(uint8_t pipe_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Requesting Host id for pipe 0x%x", fn, pipe_id);
  int i;

  for (i = 0; i < mPipesInfo[DH_IDX].nb_pipes; i++) {
    if (mPipesInfo[DH_IDX].data[i].pipe_id == pipe_id) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Pipe 0x%x belongs to host 0x%x", fn, pipe_id,
...
uint8_t NfcStExtensions::checkGateForHostId(uint8_t gate_id, uint8_t host_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Checking if gate 0x%x exists between DH and host 0x%x", fn,
          gate_id, host_id);
  if (host_id == DH_ID) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: Requested host shall not be DH ", fn);
    return 0xFF;
  }

  for (i = 0; i < mPipesInfo[DH_IDX].nb_pipes; i++) {
    if ((mPipesInfo[DH_IDX].data[i].dest_gate == gate_id) && ((mPipesInfo[DH_IDX]
            .data[i].dest_host
                                                               == host_id)
                                                              || (mPipesInfo[DH_IDX]
                                                                      .data[i]
                                                                  ...
                                                                  == host_id))) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Found gate 0x%x between DH and host 0x%x - pipe_id = 0x%x",
              fn, gate_id, host_id, mPipesInfo[DH_IDX].data[i].pipe_id);
 ...
 **
 ** Description:     Set the CLF configuration to setup the CE on SwitchOFF if needed.
 **
 ** Returns:         None.
 **
 *******************************************************************************/
...
void NfcStExtensions::updateSwitchOffMode() {
  ...
  setProprietaryConfigSettings(NFCC_CONFIGURATION, 0, 0, !mCeOnSwitchOffState); //real byte number computed in there
 ...
 **
 *******************************************************************************/
NfcStExtensions& NfcStExtensions::getInstance() {
  return sStExtensions;
}
...
int NfcStExtensions::prepareGate(uint8_t gate_id) {
  ...
  if (gateInfo->added == false) {
    //Add static pipe to accept notify pipe created from NFCC at eSE initial activation
  ...
  if (gateInfo->created == false) {
    ...
    if (gateInfo->opened == false) {
      ...
      if ((nfaStat = NFA_HciOpenPipe(mNfaStExtHciHandle, gateInfo->pipe_id)) == NFA_STATUS_OK) {
...
int NfcStExtensions::prepareGateForTest(uint8_t gate_id, uint8_t host_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: Preparing gate 0x%x for test on host 0x%x", fn, gate_id,
          host_id);
  ...
  if (isGateSetup == false) {
    ...
    if (nfaStat != NFA_STATUS_OK) {
      LOG(ERROR) << StringPrintf(
              "%s: fail adding static pipe for gate 0x%x; error=0x%X", fn,
              nfaStat, gate_id);
      return (0xFF);
    }
  }
  ...
  if ((isPipeOpened == false) && (mCreatedPipeId != 0xFF)) {
    ...
    if ((nfaStat = NFA_HciOpenPipe(mNfaStExtHciHandle, mCreatedPipeId)) == NFA_STATUS_OK) {
...
void NfcStExtensions::nfaHciCallback(tNFA_HCI_EVT event,
  ...
  switch (event) {
    ...
    case NFA_HCI_CREATE_PIPE_EVT: {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_HCI_CREATE_PIPE_EVT; status=0x%X; pipe=0x%X; src gate=0x%X; dest host=0x%X; dest gate=0x%X",
    ...
    case NFA_HCI_GET_REG_RSP_EVT: {
      ...
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_HCI_GET_REG_RSP_EVT; status: 0x%X; pipe: 0x%X, reg_idx: 0x%X, len: %d",
        ...
        if (eventData->registry.status == NFA_STATUS_OK) {
          if (eventData->registry.index == VERSION_SW_REG_IDX) {
            for (i = 0; i < eventData->registry.data_len; i++) {
              sStExtensions.mIdMgmtInfo.version_sw[i] = eventData->registry
                      .reg_data[i];
            }
          } else if (eventData->registry.index == VERSION_HW_REG_IDX) {
            for (i = 0; i < eventData->registry.data_len; i++) {
              sStExtensions.mIdMgmtInfo.version_hw[i] = eventData->registry
                      .reg_data[i];
            }
          }
        }
    ...
    case NFA_HCI_SET_REG_RSP_EVT: {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_HCI_SET_REG_RSP_EVT; status: 0x%X; pipe: 0x%X, reg_idx: 0x%X, len: %d",
              fn, eventData->registry.status, eventData->registry.pipe,
              eventData->registry.index, eventData->registry.data_len);
    }
      break;
    ...
    case NFA_HCI_RSP_RCVD_EVT: { //response received from secure element
      ...
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_HCI_RSP_RCVD_EVT; status: 0x%X; code: 0x%X; pipe: 0x%X; len: %u",
        ...
        if (rsp_rcvd.rsp_code == NFA_HCI_ANY_OK) {
          if (sStExtensions.mIsWaitingEvent.propHciRsp == true) { //data reception
            ...
            for (i = 0; i < rsp_rcvd.rsp_len; i++) {
              LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                      "%s: NFA_HCI_RSP_RCVD_EVT; sp_rcvd.rsp_data[%d] = 0x%x",
                      fn, i, rsp_rcvd.rsp_data[i]);
    ...
    case NFA_HCI_EVENT_SENT_EVT:
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_HCI_EVENT_SENT_EVT; status=0x%X", fn,
...
int NfcStExtensions::getFirmwareVersion(uint8_t* fwVersion) {
  ...
  fwVersion[3] = mFwInfo;
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: FW Version = %02X.%02X.%02X%02X", fn, fwVersion[0], fwVersion[1],
          fwVersion[2], fwVersion[3]);
  /* NCI 2.0 - End */
...
int NfcStExtensions::getHWVersion(uint8_t* hwVersion) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: HW Version = %02X%02X ",
                                                  fn, hwVersion[0],
                                                  hwVersion[1]);
  /* NCI 2.0 - End */
 ...
 *******************************************************************************/
 bool NfcStExtensions::isSEConnected(int se_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: enter, requesting info for SE id 0x%x", fn, se_id);
  bool result = false;
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: SE id 0x%x connected is %d", fn, se_id, result);
...
uint32_t NfcStExtensions::getNfcSystemProp(const char* key_id) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s - Read property %s - len = %d - value = %lu", fn, key_id, len,
          num);
...
void NfcStExtensions::setRfConfiguration(int modeBitmap, uint8_t* techArray) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter - mIsReaderMode = %d",
      fn, mIsReaderMode);

  tNFA_STATUS nfaStat = NFA_STATUS_FAILED;
  bool mustRestartDiscovery = false;

  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: modeBitmap = 0x%x", fn,
                                                  modeBitmap);
  for (int i = 0; i < RF_CONFIG_ARRAY_SIZE; i++) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: techArray[%d] = 0x%x",
                                                    fn, i, techArray[i]);
  }
  ...
  if (android::isDiscoveryStarted()) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Stopping RF discovery",
                                                    fn);
    //Stop RF discovery
  ...
  //P2P listen
  {
    if ((mRfConfig.techArray[P2P_LISTEN_IDX]) && (mRfConfig.modeBitmap
            & (0x1 << P2P_LISTEN_IDX))) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Cleaning p2p listen tech", fn);
    }
  ...
  //Listen
  {
    if ((mRfConfig.techArray[CE_IDX]) && (mRfConfig.modeBitmap & (0x1 << CE_IDX))) {
      {
        LOG_IF(INFO, nfc_debug_enabled)
...
#define MAX_NUM_EE 5
  ...
  if ((modeBitmap & (0x1 << READER_IDX)) || (modeBitmap
          & (0x1 << P2P_LISTEN_IDX))) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Reprogram polling",
                                                    fn);
  ...
  if (modeBitmap & (0x1 << CE_IDX)) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Reprogram listen mode",
                                                    fn);
    {
      if (techArray[CE_IDX] != 0) {
          if ((nfaStat = NFA_CeSetIsoDepListenTech(techArray[CE_IDX])) != NFA_STATUS_OK) { //nothing ot wait here
            LOG(ERROR) << StringPrintf(
                ...
                "%s: Re-enable listen mode", fn);
        {
          ...
          if ((nfaStat = NFA_EnableListening()) == NFA_STATUS_OK) {
            ...
            LOG(ERROR) << StringPrintf(
                    "%s: NFA_EnableListening() failed; error=0x%X", fn,
                    nfaStat);
            gIsReconfiguringDiscovery.end();
            return;
          }
  ...
  if (modeBitmap & (0x1 << P2P_LISTEN_IDX)) { //program p2p listen
    {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Reprogram p2p listen", fn);
      PeerToPeer::getInstance().setP2pListenMask(techArray[P2P_LISTEN_IDX]);
      PeerToPeer::getInstance().enableP2pListening(true);
    }
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Re-enable listen mode",
                                                    fn);
    {
      SyncEventGuard guard(mNfaDmEventListenEnabled);
      if ((nfaStat = NFA_EnableListening()) == NFA_STATUS_OK) {
...
int NfcStExtensions::getRfConfiguration(uint8_t* techArray) {
  ...
  for (i = 0; i < RF_CONFIG_ARRAY_SIZE; i++) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: mRfConfig.techArray[%d] = 0x%x", fn, i,
            mRfConfig.techArray[i]);
  }
...
void NfcStExtensions::setRfBitmap(int modeBitmap) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: enter - modeBitmap: %02X", fn, modeBitmap);
  tNFA_STATUS nfaStat = NFA_STATUS_FAILED;
  ...
  // Polling
  {
    if (((modeBitmap & (0x1 << READER_IDX)) == 0) ||
      ...
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Not cleaning polling tech", fn);
    }
  }
  ...
  //P2P listen
  {
    if ((modeBitmap & (0x1 << P2P_LISTEN_IDX)) == 0) {
      ...
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: Not cleaning p2p listen tech", fn);
    }
  }
 ...
 **
 *******************************************************************************/
//void NfcStExtensions::notifyTransactionListenersOfAidSelection (const uint8_t* aidBuffer, uint8_t aidBufferLen, uint8_t host_id)
//{
//    static const char fn [] = "NfcStExtensions::notifyTransactionListenersOfAid";
//    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; aid len=%u", fn, aidBufferLen);
//
//    if (aidBufferLen == 0) {
//        return;
//    }
...
//    }
//
//    e->CallVoidMethod (mNativeData->manager, android::gCachedNfcManagerNotifyAidSelectionReceived, tlvJavaArray.get(),(jint)host_id);
//    if (e->ExceptionCheck()) {
//        e->ExceptionClear();
...
void NfcStExtensions::setDefaultOffHostRoute(int route) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter - route = 0x%x",
                                                  fn, route);
...
void NfcStExtensions::setProprietaryConfigSettings(int prop_config_id,
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: enter - byteNb = 0x%x, bitNb = 0x%x, ValueToSet = %d", fn,
          byteNb, bitNb, status);
  ...
  //Before Updating configuration, read it to update content of mNfccConfig.config[]
  currentStatus = getProprietaryConfigSettings(prop_config_id, byteNb, bitNb); //real byte number computed in there
  ...
  if (mPropConfigLen == 0) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: No parameters returned - Exit", fn);
    return;
  }
...
void NfcStExtensions::setP2pPausedStatus(bool status) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: enter; current status is mIsP2pPaused = %d, new status is %d",
          fn, mIsP2pPaused, status);
 ...
 *******************************************************************************/bool NfcStExtensions::getP2pPausedStatus() {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: enter; mIsP2pPaused = %d", fn, mIsP2pPaused);
...
int NfcStExtensions::getATR(uint8_t* atr)
{
    ...
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: ATR length = %d;", fn, length);
...
int NfcStExtensions::connectGate(int gate_id, int host_id) {
  ...
  if (pipe_id != 0xFF) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: Allocated pipe 0x%x for gate 0x%x on host 0x%x", fn, pipe_id,
            gate_id, host_id);
  } else {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: Pipe creation failed for gate 0x%x on host 0x%x", fn, gate_id,
            host_id);
  }
 ...
 **
 *******************************************************************************/
bool NfcStExtensions::transceive(
        uint8_t pipeId, uint8_t hciCmd, uint16_t txBufferLength,
        uint8_t* txBuffer, int32_t& recvBufferActualSize, uint8_t* rxBuffer) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: received data of length 0x%02X", fn, mRxHciDataLen);
...
bool NfcStExtensions::checkListenAPassiveOnSE() {
  ...
  for (i = 0; i < 2; i++) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: Checking if %02X is connected", fn, seList[i]);
    if (isSEConnected(seList[i]) == true) {
      bitmap |= SE_CONNECTED_MASK;
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: %02X is connected, check presence of A Card RF gate/pipe",
              fn, seList[i]);
  ...
  //No A Card RF gate/pipe found, remove Listen A passive mode from P2P polling tech
...
void NfcStExtensions::notifyNciConfigCompletion(bool isGet, uint16_t length,
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: isGet = %d, length = 0x%02X", fn, isGet, length);
 ...
 **
 *******************************************************************************/
void NfcStExtensions::sendPropGetConfig(int subSetId, int configId, uint8_t *param, uint16_t& length)
{
    static const char fn[] = "NfcStExtensions::sendPropGetConfig";
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter (subSetId=0x%x)", fn,subSetId);
    ...
    nfaStat = NFA_SendVsCommand(OID_ST_VS_CMD, 5, getPropConfig, nfaVsCbActionRequest);
    if (nfaStat != NFA_STATUS_OK) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_SendVsCommand() call failed; error=0x%X", fn, nfaStat);
    } else {
        mVsActionRequestEvent.wait();
        mIsWaitingEvent.getPropConfig = false;

        length = mPropConfigLen;
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: data received, length=%d", fn,length);
 ...
 **
 ** Description:    Force new RF configuration. Only available for CustomA/CustomB.
 **
 ** Returns:         void
 **
 *******************************************************************************/
          ...
          switch (p_data[0]) {
            case T_dynParamUsed: {
              ...
              if (fw_cur_set != sRfDynParamSet) {
                LOG_IF(INFO, nfc_debug_enabled) << fn
 ...
 **
 ** Description:    Internal functions to manage the reconstruction of fake ACTION_NTF.
 **
 ** Returns:         void
 **
 *******************************************************************************/
// Send one event to service
void NfcStExtensions::StMatchSendTriggerPayload(uint8_t nfcee, uint8_t * buf, int len) {
  static const char fn[] = "NfcStExtensions::StMatchSelectSw::SendTrigger";
  JNIEnv* e = NULL;
  ScopedAttach attach(mNativeData->vm, &e);
  LOG_IF(INFO, nfc_debug_enabled) << fn
        << StringPrintf(": send (%x) %db: %02x%02x...", nfcee, len, buf[0], buf[1]);
  if (e == NULL) {
    LOG(ERROR) << StringPrintf("jni env is null");
    return;
  }
  ScopedLocalRef<jbyteArray> ntfData(e, e->NewByteArray(len));
  e->SetByteArrayRegion(ntfData.get(), 0, len,
                          (jbyte*)(buf));

  e->CallVoidMethod(mNativeData->manager,
                    android::gCachedNfcManagerNotifyActionNtf,
                    (jint)nfcee, ntfData.get());
}

// We received an ACTION (AID) ntf from CLF
void NfcStExtensions::StMatchStoreActionAid(uint8_t nfcee, uint8_t * aid, int len) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << fn
        << StringPrintf(": store (%x) %db: %02x%02x...%02x",
            nfcee,
            len,
            (len > 0) ? aid[0] : 0,
            (len > 1) ? aid[1] : 0,
...
void NfcStExtensions::StMatchGotLogPartialAid(uint8_t * aidBeg, int aidBegLen,
            uint8_t * aidEnd, int aidEndLen, int fullLen) {
  static const char fn[] = "NfcStExtensions::StMatchSelectSw::StorePartialAid";
  SyncEventGuard guard(mMatchSelectLock);
  LOG_IF(INFO, nfc_debug_enabled) << fn
      << StringPrintf(": Matched SELECT,  %d(%d)b: %02x%02x...%02x%02x",
      fullLen, aidBegLen+aidEndLen,
...
void NfcStExtensions::StMatchGotLogSw(uint8_t sw1, uint8_t sw2) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << fn
      << StringPrintf(": Got SW %02x%02x", sw1, sw2);
  ...
  for (i = 0; i < mMatchSelectedCurrent; i++) {
    if ((mMatchSelectedAidLen[i] == mMatchSelectPartialOrigLen)
        && (!mMatchSelectPartialAidBegLen || !memcmp(&mMatchSelectedAid[16*i], mMatchSelectPartialAid, mMatchSelectPartialAidBegLen))
        && (!mMatchSelectPartialAidEndLen || !memcmp(&mMatchSelectedAid[16*i + mMatchSelectedAidLen[i] - mMatchSelectPartialAidEndLen], mMatchSelectPartialAid  + mMatchSelectPartialAidBegLen, mMatchSelectPartialAidEndLen))
        ) {
          LOG_IF(INFO, nfc_debug_enabled) << fn
                << StringPrintf(": matched=#%d", i);
          break;
    }
  }
  ...
  if (i < mMatchSelectedCurrent) {
    ...
    if (fakeTrig[1]) memcpy(fakeTrig+2, &mMatchSelectedAid[16 * i], fakeTrig[1]);
    ...
    LOG_IF(INFO, nfc_debug_enabled) << fn
        << StringPrintf(": No matching ACTION NTF, save log data.");
...
void NfcStExtensions::StMatchPurgeActionAid(int num, bool skipLast) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << fn
        << StringPrintf(": purged %d(real:%d) remains %d", num, i, mMatchSelectedCurrent);
}
 ...
 **
 *******************************************************************************/
void NfcStExtensions::StMatchSelectSw(uint8_t format,
                                               uint16_t data_len,
                                               uint8_t* p_data, bool last) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << fn
      << StringPrintf(": processing t:%02x l:%d", t, l);
  ...
  switch (mMatchSelectState) {
          ...
          switch (p_data[2] & 0xF) {
            case 0x7: { // B standard frame.
              LOG_IF(INFO, nfc_debug_enabled) << fn
                  << ": Rx type B";
              // SENSB_REQ/ALLB_REQ
              if ((p_data[offset + 2] == 0x05) && (p_data[offset + 3] == 0x00)) {
                LOG_IF(INFO, nfc_debug_enabled) << fn
                    << ": SENSB_REQ/ALLB_REQ";
                return;
              }
    ...
    case MATCH_SEL_ST_CE_IN_ISODEP: {
      ...
      switch (t) {
        ...
        case T_CERx: {
          ...
          LOG_IF(INFO, nfc_debug_enabled) << fn
              << StringPrintf(": realLen=%d", realLen);
          offset += 2;
          ...
          LOG_IF(INFO, nfc_debug_enabled) << fn
              << StringPrintf(": INS=0x%02hhx", p_data[offset + 1]);
          ...
          if (p_data[offset + 1] == 0xA4) {
            ...
            uint8_t tailLen = realLenDataStart + realLen - realAidLenStart - realAidLen;

            if (tailLen > 1) {
              LOG_IF(INFO, nfc_debug_enabled) << fn
                  << StringPrintf(": tailLen=0x%02hhx skip", tailLen);
              return;
            }
            if (realAidLenStart + realAidLen + tailLen > data_len) {
              // the data is truncated, we have the last 2 bytes.
              StMatchGotLogPartialAid(p_data+realAidLenStart, data_len-2 - (realAidLenStart),
                p_data+data_len-2, 2 - tailLen, p_data[offset + 4]);
            } else {
              // the AID is complete.
              StMatchGotLogPartialAid(p_data + realAidLenStart, p_data[offset + 4],
                NULL, 0, p_data[offset + 4]);
            }
    ...
    case MATCH_SEL_ST_CE_GOT_SELECT: {
      ...
      switch (t) {
        ...
        case T_RxI: {
          ...
          if (pid >= 0x21 && pid <= 0x24) {
            if (!has_cb) {
              ...
              uint8_t sw1, sw2;
              offset++;
              // if only 1 byte payload, this was frag, otherwise SW in last 2 bytes.
        ...
        //   } else {
        //     uint8_t sw1, sw2;
        //     // if only 1 byte INF, this was frag, otherwise SW in last 2 bytes.
        //     if (offset + 1 == data_len) {
        //       SyncEventGuard guard(mMatchSelectLock);
        //       LOG_IF(INFO, nfc_debug_enabled) << fn << ": split SW";
        //       sw1 = mMatchSelectPartialLastChainedByte;
...
**
*******************************************************************************/
void NfcStExtensions::StVsLogsCallback (tNFC_VS_EVT event, uint16_t data_len, uint8_t* p_data)
{
    static const char fn [] = "StLogManager::StVsLogsCallback";
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s : event = 0x%02X", fn, event);
    if (data_len < 6) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s : data_len = 0x%X", fn, data_len);
      return;
    }
    if (p_data[1] != 0x02 || p_data[4] != 0x20) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s : 1:3:4 = 0x%02X:0x%02X:0x%02X", fn, p_data[1], p_data[3], p_data[3]);
...
void NfcStExtensions::StAidTriggerActionCallback(tNFA_EE_ACTION & action) {
  ...
  if (doSendUpper) {
    ...
    switch (buffer[0]) {
      case NCI_EE_TRIG_7816_SELECT: {
        ...
        if (1) { // change to revert to normal AID triggers
          StMatchStoreActionAid(nfcee, action.param.aid.aid, action.param.aid.len_aid);
          buflen = 0;
        }
      } break;
    ...
    if (buflen > 0) {
      ...
      e->SetByteArrayRegion(ntfData.get(), 0, buflen,
                              (jbyte*)(buffer));
