...
static jbyteArray nativeNfcStExtensions_getFirmwareVersion(JNIEnv* e, jobject)
{
...

    {
        ...
        if (result != NULL) {
            e->SetByteArrayRegion(result, 0, NfcStExtensions::getInstance().FW_VERSION_SIZE, (jbyte *) version_sw);
        }
    }
...
static jbyteArray nativeNfcStExtensions_getHWVersion(JNIEnv* e, jobject)
{
...

    {
        ...
        if (result != NULL) {
            e->SetByteArrayRegion(result, 0, NfcStExtensions::getInstance().HW_VERSION_SIZE, (jbyte *) version_hw);
        }
    }
...
**
*******************************************************************************/
static jbyteArray nativeNfcStExtensions_getCRCConfiguration(JNIEnv* e, jobject)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jbyteArray result = NULL;
    ...
    // if (ret)
    {
        ...
        if (result != NULL) {
            e->SetByteArrayRegion(result, 0, NfcStExtensions::getInstance().CRC_CONFIG_SIZE, (jbyte *) crc_config);
        }
    }
...
static jbyteArray nativeNfcStExtensions_getCustomerData(JNIEnv* e, jobject)
{
...

    {
        ...
        if (result != NULL) {
            e->SetByteArrayRegion(result, 0, sizeof(customerData), (jbyte *) customerData);
        }
    }
...
//    char hal_name[NAME_MAX];
    ...
    /* if (property_get("ro.product.board", hal_name, NULL) == 0)
     {
         DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: ro.product.board not defined, cannot find hal library", __func__);
     }
     else*/
    //{

        //strcat (hal_path,"hw/nfc_nci.st21nfc.default.so");
        //DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: loading hal library %s", __func__, hal_path);
...
//    len = strlen(*temp);
 ...
 //   dlclose(handle);
 //   DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: HAL version is %s", __func__, hal_version);
 //   res_len += len;
    ...
    dlclose(handle);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Core stack version is %s", __func__, core_stack_version);
    res_len += len;

    //Get NfcService/JNI version -idx 2 => JNIVersion in this file
    len = strlen(JNIVersion);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: JNI version is %s;", __func__, JNIVersion);
    res_len += len;
    ...
    dlclose(handle);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Android version is %s", __func__, android_version);
    res_len += len;
    ...
    //DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Found length is %d", __func__, (len+3));
...
**
*******************************************************************************/
static jboolean nativeNfcStExtensions_isSEConnected(JNIEnv* e, jobject, jint HostID)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jboolean result;
...
**
*******************************************************************************/
static void nativeNfcStExtensions_setRfConfiguration(JNIEnv* e, jobject, jint modeBitmap, jbyteArray techArray)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ScopedByteArrayRW bytes(e, techArray);
    NfcStExtensions::getInstance().setRfConfiguration(modeBitmap, reinterpret_cast<uint8_t*>(&bytes[0]));
}
...
**
*******************************************************************************/
static int nativeNfcStExtensions_getRfConfiguration(JNIEnv* e, jobject, jbyteArray techArray)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ...
    e->SetByteArrayRegion(techArray, 0, NfcStExtensions::getInstance().RF_CONFIG_ARRAY_SIZE, (jbyte *) outputArray);

    return modeBitmap;
}

static void nativeNfcStExtensions_setRfBitmap(JNIEnv* e, jobject, jint modeBitmap)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
...
static jint nativeNfcStExtensions_updatePipesInfo(JNIEnv* e, jobject o)
{
    ...
    if (res == true) {
        pipe_id = NfcStExtensions::getInstance().getPipeIdForGate(NfcStExtensions::getInstance().DH_HCI_ID, 0x41);
...
**
*******************************************************************************/
static void nativeNfcStExtensions_setDefaultOffHostRoute(JNIEnv* e, jobject, jint route)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
...
*******************************************************************************/
/* NCI 2.0 - Begin */
static jboolean nativeNfcStExtensions_getProprietaryConfigSettings(JNIEnv* e, jobject, jint SubSetId, jint byteNb, jint bitNb)
/* NCI 2.0 - End */
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    bool status = false;
    ...
    /* NCI 2.0 - Begin */
    status = NfcStExtensions::getInstance().getProprietaryConfigSettings(SubSetId ,byteNb, bitNb);
    /* NCI 2.0 - End */
...
*******************************************************************************/
/* NCI 2.0 - Begin */
static void nativeNfcStExtensions_setProprietaryConfigSettings(JNIEnv* e, jobject, jint SubSetId,jint byteNb, jint bitNb, jboolean status)
/* NCI 2.0 - End */
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);

    /* NCI 2.0 - Begin */
    NfcStExtensions::getInstance().setProprietaryConfigSettings(SubSetId,byteNb, bitNb, status);
    /* NCI 2.0 - End */
...
**
*******************************************************************************/
static jint nativeNfcStExtensions_getPipesList(JNIEnv* e, jobject, jint host_id, jbyteArray list)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: nb_pipes: %d", __func__, nb_pipes);

    uint8_t listArray[nb_pipes];

    for(i=0; i<nb_pipes; i++) {
        listArray[i] = NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].pipe_id;
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: listArray[%d]= 0x%x", __func__, i, listArray[i]);
    }
...
**
*******************************************************************************/
static void nativeNfcStExtensions_getPipeInfo(JNIEnv* e, jobject, jint host_id, jint pipe_id, jbyteArray info)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    int i, nb_pipes = 0, host_idx;
    ...
    if(nb_pipes == 0xFF) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: getPipesInfo() was not called!!;", __func__);
        return;
    }
    ...
    for(i=0; i<nb_pipes; i++) {
        if(NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].pipe_id == pipe_id) {
            infoArray[0] = NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].pipe_state;
            infoArray[1] = NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].source_host;
            infoArray[2] = NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].source_gate;
            infoArray[3] = NfcStExtensions::getInstance().mPipesInfo[host_idx].data[i].dest_host;
            break;
        }
    }
...
**
*******************************************************************************/
static jboolean nativeNfcStExtensions_EnableSE(JNIEnv* e, jobject, jint se_id, jboolean enable)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jboolean result;
...
**
*******************************************************************************/
static jboolean nativeNfcStExtensions_connectEE(JNIEnv* e, jobject, jint cee_id)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jboolean result;
...
**
*******************************************************************************/
static jbyteArray nativeNfcStExtensions_transceiveEE(JNIEnv* e, jobject, jint cee_id, jbyteArray data_cmd)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ...
    rslt = StCardEmulationEmbedded::getInstance().transceive(cee_id, bytes.size(), reinterpret_cast<uint8_t*>(&bytes[0]), recvBufferActualSize, recvBuffer);
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit: received data length =%d", __func__, recvBufferActualSize);
    return result;
}
...
**
*******************************************************************************/
static jboolean nativeNfcStExtensions_disconnectEE(JNIEnv* e, jobject, jint cee_id)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jboolean result;
...
**
*******************************************************************************/
static jint nativeNfcStExtensions_connectGate(JNIEnv* e, jobject, jint host_id, jint gate_id)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    jint result;
...
**
*******************************************************************************/
static jbyteArray nativeNfcStExtensions_transceive(JNIEnv* e, jobject, jint pipe_id, jint hciCmd, jbyteArray data_cmd)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ...
    rslt = NfcStExtensions::getInstance().transceive(pipe_id, hciCmd, bytes.size(), reinterpret_cast<uint8_t*>(&bytes[0]), recvBufferActualSize, recvBuffer);
...
**
*******************************************************************************/
static void nativeNfcStExtensions_disconnectGate(JNIEnv* e, jobject, jint pipe_id)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
...
**
*******************************************************************************/
static void nativeNfcStExtensions_setNciConfig(JNIEnv* e, jobject, jint param_id, jbyteArray param, jint length)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);

    ScopedByteArrayRW bytes(e, param);
    NfcStExtensions::getInstance().setNciConfig(param_id, reinterpret_cast<uint8_t*>(&bytes[0]), length);
}
...
**
*******************************************************************************/
static jbyteArray nativeNfcStExtensions_getNciConfig(JNIEnv* e, jobject, jint param_id)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    uint16_t recvBufferActualSize = 0;
    uint8_t recvBuffer [256];

    NfcStExtensions::getInstance().getNciConfig(param_id, recvBuffer, recvBufferActualSize);
...
**
*******************************************************************************/
static int nativeNfcStExtensions_getAvailableHciHostList(JNIEnv* e, jobject, jbyteArray nfceeId, jbyteArray conInfo)
{
    uint8_t nfceeIdArray[3];
    uint8_t conInfoArray[3];
    int num =0;

    num= NfcStExtensions::getInstance().getAvailableHciHostList(nfceeIdArray, conInfoArray);

    for(int i=0; i<num; i++) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: nfceeIdArray[%d] - 0x%02x", __FUNCTION__, i, nfceeIdArray[i]);
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: conInfoArray[%d] - 0x%02x", __FUNCTION__, i, conInfoArray[i]);
    }
...
static void nativeNfcStExtensions_sendPropSetConfig(JNIEnv* e, jobject, jint subSetId, jint configId, jbyteArray param, jint length)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);

    ScopedByteArrayRW bytes(e, param);
    NfcStExtensions::getInstance().sendPropSetConfig(subSetId, configId, reinterpret_cast<uint8_t*>(&bytes[0]), length);
}

static jbyteArray nativeNfcStExtensions_sendPropGetConfig(JNIEnv* e, jobject, jint subSetId, jint configId)
{
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    ...
    NfcStExtensions::getInstance().sendPropGetConfig(subSetId, configId, recvBuffer, recvBufferActualSize);
...
static jbyteArray nativeNfcStExtensions_sendPropTestCmd(JNIEnv* e, jobject, jint subCode, jbyteArray paramTx, jint lengthTx)
{
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter;", __func__);
    uint16_t recvBufferActualSize = 0;
    uint8_t recvBuffer [256];
    ...
    NfcStExtensions::getInstance().sendPropTestCmd(subCode, reinterpret_cast<uint8_t*>(&bytes[0]), lengthTx, recvBuffer, recvBufferActualSize);
...
static JNINativeMethod gMethods[] = {
    {"getFirmwareVersion", "()[B", (void *) nativeNfcStExtensions_getFirmwareVersion},
    {"getCRCConfiguration", "()[B", (void *) nativeNfcStExtensions_getCRCConfiguration},
    ...
    {"setRfConfiguration", "(I[B)V", (void *) nativeNfcStExtensions_setRfConfiguration},
    {"getRfConfiguration", "([B)I", (void *) nativeNfcStExtensions_getRfConfiguration},
    {"setRfBitmap", "(I)V", (void *) nativeNfcStExtensions_setRfBitmap},
    {"updatePipesInfo", "()I", (void *) nativeNfcStExtensions_updatePipesInfo},
    {"setDefaultOffHostRoute", "(I)V", (void *) nativeNfcStExtensions_setDefaultOffHostRoute},
    {"getDefaultOffHostRoute", "()I", (void *) nativeNfcStExtensions_getDefaultOffHostRoute},
    /* NCI 2.0 - Begin */
    {"getProprietaryConfigSettings", "(III)Z", (void *) nativeNfcStExtensions_getProprietaryConfigSettings},
    {"setProprietaryConfigSettings", "(IIIZ)V", (void *) nativeNfcStExtensions_setProprietaryConfigSettings},
    /* NCI 2.0 - End */
    ...
    {"getAvailableHciHostList", "([B[B)I", (void *) nativeNfcStExtensions_getAvailableHciHostList},
    {"sendPropSetConfig", "(II[BI)V", (void *) nativeNfcStExtensions_sendPropSetConfig},
    {"sendPropGetConfig", "(II)[B", (void *) nativeNfcStExtensions_sendPropGetConfig},
    {"sendPropTestCmd", "(I[BI)[B", (void *) nativeNfcStExtensions_sendPropTestCmd},
...
{
    return jniRegisterNativeMethods(e, gNativeNfcStExtensionsClassName,
