...
/////////////////////////////////////////////////////////////
// Wallet API
static bool nfcManager_SetMuteTech(JNIEnv* e, jobject o, jboolean muteA, jboolean muteB, jboolean muteF,
...
static void handleRfDiscoveryEvent(tNFC_RESULT_DEVT* discoveredDevice) {
  ...
  if (NULL == discoveredDevice) {
    LOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: parameter discoveredDevice can not be null error",
            __func__);
    return;
  }
  ...
  if (gEnableSkipMifare == true) {
    if (discoveredDevice->protocol == NFC_PROTOCOL_ISO_DEP) {
      LOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: ISO-DEP, rfDiscId = %d", __func__,
      ...
      LOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: MIFARE, rfDiscId = %d", __func__,
              discoveredDevice->rf_disc_id);
  ...
  if ((gEnableSkipMifare == true) && (isoMifareBitmap == 0x03) &&
    ...
    LOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Same tag discovered twice, skip MIFARE detection",
            __func__);
  ...
  if (!sReaderModeEnabled && isP2p) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf(
            "%s: Reader mode enabled and P2P detected but paused, cancel multidetection",
            __func__);
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Only one tag detected, skip multitag detection",
            __func__);
    NfcTag::getInstance().mNumDiscNtf = 0x00;
    NfcTag::getInstance().mIsMultiProtocolTag = false;
    NfcTag::getInstance().selectFirstTag();
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: starting p2p prio logic for multiprotocol tags",
            __func__);
...
//    {
    gIsReconfiguringDiscovery.start();
...

        {
            ...
            DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf ("%s: Failed to disable polling; error=0x%X", __func__, status);
        }
      ...
      << StringPrintf ("%s: re-configure polling to default", __func__);
			...
			tech_mask = NfcConfig::getUnsigned(NAME_POLLING_TECH_MASK, DEFAULT_TECH_MASK);
        }
...
void reconfigure_poll_cb(union sigval)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - CallBack Reconfiguring the POLL to Default", __func__);
    clear_multiprotocol();
...
static void nfcManager_isSkipMifareInterface() {
  ...
  if ((isoMifareBitmap == 0x03) && (gEnableSkipMifare == true)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: ISO+MIFARE tag, "
        "Set flag to skip MIFARE interface", __func__);
  ...
  switch (connEvent) {
    ...
    case NFA_DISC_RESULT_EVT:  // NFC link/protocol discovery notificaiton
    {
      status = eventData->disc_result.status;
            DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: NFA_DISC_RESULT_EVT: status = 0x%0X", __func__, status);
          ...
          << StringPrintf("%s: NFA_DISC_RESULT_EVT: Success", __func__);
            ...
            {
                DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: NFA_DISC_RESULT_EVT: reSelect function didn't save the modification", __func__);
                if(cur_more_val == 0x00)
                {
                    DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: NFA_DISC_RESULT_EVT: error, select any one tag", __func__);
                    multiprotocol_flag = 0;
                }
            }
    ...
    case NFA_ACTIVATED_EVT:  // NFC link/protocol activated
    {
      ...
      uint8_t activatedProtocol =
          (tNFA_INTF_TYPE)eventData->activated.activate_ntf.protocol;
            ...
            {
                LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("Prio_Logic_multiprotocol stop timer");
                multiprotocol_timer.kill();
            }

            if( (eventData->activated.activate_ntf.protocol == NFA_PROTOCOL_T3T) && (multiprotocol_detected == 1) )
            {
                LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("T3T tag detected, Prio_Logic_multiprotocol stop timer and exit");
                multiprotocol_timer.kill();
                clear_multiprotocol();
                break;
            }
            ...
            if(isIsoMifareFlag){
                LOG_IF(INFO, nfc_debug_enabled) <<
                        StringPrintf("nfaConnectionCallback(NFA_ACTIVATED_EVT) - ISO + MIFARE stop timer");
                isIsoMifareFlag = false;
      ...
      if (isPeerToPeer(eventData->activated)) {
        if (sReaderModeEnabled) {
          ...
          break;
                   ...
                   true) {
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: P2P is paused, deactivating", __func__);
          NFA_Deactivate(FALSE);
          break;
        }
        sP2pActive = true;
        ...
        // Workaround : Notify a field OFF to avoid the UI stays stuck on Field
        // ON last event
        {
       ...
       LOG_IF(INFO, nfc_debug_enabled)
                << StringPrintf("%s: scoreGenericNtf = 0x%x", __func__ ,scoreGenericNtf);

        if(scoreGenericNtf == true)
        {
            if( (eventData->activated.activate_ntf.intf_param.type == 
            NFC_INTERFACE_ISO_DEP) && 
            (eventData->activated.activate_ntf.protocol == NFC_PROTOCOL_ISO_DEP) )
        {
...
//        }
    ...
    case NFA_DEACTIVATED_EVT:  // NFC link/protocol deactivated
      ...
      NfcTag::getInstance().setDeactivationState(eventData->deactivated);
		...
		DLOG_IF(INFO, nfc_debug_enabled) << 
			StringPrintf("%s - nb of discovered ntf: %d", __func__, NfcTag::getInstance ().mNumDiscNtf);
...
static jint StNfcManager_doRegisterT3tIdentifier(JNIEnv* e, jobject,
  ...
  uint8_t* buf =
      const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(&bytes[0]));
  size_t bufLen = bytes.size();
  int handle = StRoutingManager::getInstance().registerT3tIdentifier(buf, bufLen);
...
**
*******************************************************************************/
static void stNfcManager_enableDiscovery(JNIEnv* e, jobject o,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: enter; tech_mask = %02x, enable_host_routing: %d, "
  ...
  if (sRfEnabled ||
      (sWalletTechIsMute & ST_CE_MUTE_DISCOVERY)) {
...
static bool nfcManager_SetMuteTech(JNIEnv* e, jobject o, jboolean muteA,
    jboolean muteB, jboolean muteF,jboolean isCommitNeeded){
  bool wasStopped = false;
  LOG_IF(INFO, nfc_debug_enabled)
  ...
  gIsReconfiguringDiscovery.end();
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: success, sWalletTechIsMute = 0x%02X",
      __func__, sWalletTechIsMute);
  return true;
}
...
**
*******************************************************************************/
static void nfcManager_enableSkipMifareInterface(JNIEnv* e, jobject o, jboolean skip)
{
    LOG_IF(INFO, nfc_debug_enabled)
...
static void nfcManager_setSEFelicaCardEnabled(JNIEnv* e, jobject o, jboolean status){
  LOG_IF(INFO, nfc_debug_enabled)
                << StringPrintf("Enter :%s - status:%d", __func__, status);
  StRoutingManager::getInstance().setSEFelicaCardEnable(status);
}
...
static void stNfcManager_doSetScreenState(JNIEnv* e, jobject o,
  ...
  if ((state == NFA_SCREEN_STATE_OFF_LOCKED ||
      ...
      ((prevScreenState == NFA_SCREEN_STATE_ON_UNLOCKED) ||
          (prevScreenState == NFA_SCREEN_STATE_ON_LOCKED))&& (!sP2pActive) &&
      (!sSeRfActive)) {
 ...
 ** Description:     Set default routes as set by user through NfcSettingsAdapter
 **                  APIs.
 **
 **                  e: JVM environment.
 **                  o: Java object.
 **
 *******************************************************************************/
static void stNfcManager_setUserDefaultRoutesPref(JNIEnv* e, jobject o,
                                                  jint mifareRoute,
                                                  jint isoDepRoute,
                                                  jint felicaRoute,
                                                  jint abTechRoute,
...
static JNINativeMethod gMethods[] = {
    ...
    {"doSetMuteTech", "(ZZZZ)Z",
            (void *)nfcManager_SetMuteTech},

    {"doSetNfcSecure", "(Z)Z", (void*)stNfcManager_doSetNfcSecure},

    {"setObserverMode", "(Z)Z", (void*)nfcManager_setObserverMode},
    {"setUserDefaultRoutesPref", "(IIIIII)V", (void*)stNfcManager_setUserDefaultRoutesPref},
    {"enableStLog", "(Z)V",
            (void *)nfcManager_enableStLog},
    {"rotateRfParameters", "(Z)Z", (void*)nfcManager_rotateRfParameters},
    {"enableSkipMifareInterface", "(Z)V", (void*)nfcManager_enableSkipMifareInterface},
    {"setSEFelicaCardEnabled", "(Z)Z", (void*)nfcManager_setSEFelicaCardEnabled},
...
void pollingChanged(int discoveryEnabled, int pollingEnabled, int p2pEnabled) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: %d %d %d", __func__, discoveryEnabled, pollingEnabled, p2pEnabled);
...
void doStartupConfig() {
  ...
  if (polling_frequency.size() == 8) {
  ...
  
	{
		...
		SyncEventGuard guard(sNfaSetConfigEvent);
		tNFA_STATUS status = NFA_SetConfig(NCI_PARAM_ID_RF_FIELD_INFO, sizeof(nfa_field_info),
				&nfa_field_info[0]);
		if (status == NFA_STATUS_OK)
			sNfaSetConfigEvent.wait();
	}
