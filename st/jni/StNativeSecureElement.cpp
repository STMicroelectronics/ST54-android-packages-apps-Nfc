...
static jint stNativeNfcSecureElement_doOpenSecureElementConnection(JNIEnv*,
  ...
  if (!se.isBusy() && (se.getSENfceeId(se.ESE_ID) != 0x0)) {
    ...
    if (!seIsConnected ) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: going to enable %02hhx", __func__, nfceeId);
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: exit; return handle=0x%X", __func__, secElemHandle);
...
static jboolean stNativeNfcSecureElement_doDisconnectSecureElementConnection(
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; handle=0x%04x", __func__, handle);
  bool stat = false;
...
static jbyteArray stNativeNfcSecureElement_doTransceive(JNIEnv* e, jobject,
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; handle=0x%X; buf len=%zu", __func__, handle, bytes.size());
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit: recv len=%d", __func__, recvBufferActualSize);
  return result;
}
