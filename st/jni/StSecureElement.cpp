...
bool StSecureElement::initialize(nfc_jni_native_data* native) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Requested UICC listen mask: 0x%X", fn, mUiccListenMask);

  mMaxNbSe = NfcConfig::getUnsigned("MAX_NUMBER_OF_SE", 0x05);
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Maximum number of SE supported by NFCC: 0x%X", fn, mMaxNbSe);
...

  {
    ...
    if (nfaStat != NFA_STATUS_OK) {
      LOG(ERROR) << StringPrintf("%s: fail create static pipe for APDU gate; error=0x%X", fn,
            nfaStat);
      return (false);
    }
  }
...
bool StSecureElement::getEeInfo() {
          ...
          for (int yy = 0; yy < mEeInfo[xx].num_tlvs; yy++) {
            ...
            switch (mEeInfo[xx].ee_handle & 0xFF) {
              ...
              case 0x85:
                ...
                sSecElem.mSeInfo.info[UICC_IDX].nfceeId = (mEeInfo[xx].ee_handle
                        & 0xFF);
              ...
              case 0x86:
                if (mEeInfo[xx].ee_status == NFA_EE_STATUS_ACTIVE) {
                sSecElem.mSeInfo.info[ESE_IDX].nfceeId = (mEeInfo[xx].ee_handle
                        & 0xFF);
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit; mActualNumEe=%d, mNumEePresent=%d", fn, mActualNumEe,
        mNumEePresent);
...
void StSecureElement::getHostList() {
  ...
  if ((nfaStat = NFA_HciGetHostList(mNfaHciHandle)) == NFA_STATUS_OK) {
    ...
    LOG(ERROR) << StringPrintf("%s: NFA_HciGetHostList failed; error=0x%X", fn, nfaStat);
}
...
bool StSecureElement::isSEConnected(int se_id) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter (0x%x)", fn, se_id);
  uint8_t i;
...
// Try rel12 APDU gate
    ...
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: eSE is not connected!!", fn);
    return false;
  }
  ...
  if ((mEseApduPipeId != 0xFF) && (mEseApduGateId == ESE_ADPU_GATE_ID)) {
    ...
    if (nfaStat != NFA_STATUS_OK) {
      LOG(ERROR) << StringPrintf("%s: Couldn't allocate APDU gate %02X; error=0x%X", fn,
    ...
    nfaStat = NFA_HciAddStaticPipe(mNfaHciHandle, 0xc0, ESE_ADPU_GATE_ID,
                                   mEseApduPipeId);
    if (nfaStat != NFA_STATUS_OK) {
      LOG(ERROR) << StringPrintf("%s: Couldn't add static pipe for APDU gate; error=0x%X", fn,
            nfaStat);
...
//  tNFA_HANDLE eeHandle = mActiveeSEHandle;
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, Active eSE ID(nfceeId): 0x%04x", fn, nfceeId);
  ...
  if (mAdpuPipeInfo.created && mAdpuPipeInfo.opened) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: APDU pipe is up and running", fn);
    retVal = true;
    ...
    if (mEseApduGateId == ESE_ADPU_GATE_ID) {
      ...
      if (nfaStat != NFA_STATUS_OK) {
        LOG(ERROR) << StringPrintf("%s: failed transmitting EVT_SE_SOFT_RESET to eSE; error=0x%X",
              fn, nfaStat);
        retVal = false;
        goto TheEnd;
      }
      ...
      if (nfaStat != NFA_STATUS_OK) {
        LOG(ERROR) << StringPrintf("%s: failed getting ATR bytes from APDU gate; error=0x%X", fn,
              nfaStat);
        retVal = false;
        goto TheEnd;
      }
      ...
      if (mBwi != INVALID_BWI) {
        ...
        LOG_IF(INFO, nfc_debug_enabled) <<
                StringPrintf("%s: Waiting time value for transceiving on APDU gate is %d ms",
              fn, mTxWaitingTime);
      } else {
      ...
      /* NCI 2.0 - Begin */
      {
        ...
        {
          LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: timeout waiting for NFA_EE_POWER_CTRL_EVT", fn);
        }
      }
    ...
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: APDU pipe has not been created", fn);
    retVal = false;
  }
...
bool StSecureElement::disconnectEE(jint seID) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: seID=0x%X; handle=0x%04x", fn, seID, eeHandle);
...
bool StSecureElement::transceive(uint8_t* xmitBuffer, int32_t xmitBufferSize,
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; xmitBufferSize=%d; recvBufferMaxSize=%d; timeout=%d", fn,
...

  {
    ...
    if (nfaStat == NFA_STATUS_OK) {
          ...
          // EVT_WTX_REQUEST before timeout, restart timer.
      {
        ...
        {
          /* NCI 2.0 - Begin */
          LOG(ERROR) << StringPrintf("%s: APDU gate, EVT_TRANSMIT_DATA wait response timeout", fn);
          /* NCI 2.0 - End */
          goto TheEnd;
        }
...
TheEnd:
LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit; isSuccess: %d; recvBufferActualSize: %d", fn, isSuccess,
...
void StSecureElement::notifyModeSet(tNFA_EE_MODE_SET modeSet) {
  ...
  /* NCI 2.0 - Begin */
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_EE_MODE_SET_EVT; modeSet.status = 0x%02x, modeSet.ee_handle = 0x%X, modeSet.ee_status = 0x%02X", fn,
        modeSet.status, modeSet.ee_handle, modeSet.ee_status);
  /* NCI 2.0 - End */
...
void StSecureElement::notifyPowerCtrlRsp() {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_EE_POWER_CTRL_EVT; ", fn);
...
void StSecureElement::notifyEeStatus(tNFA_HANDLE eeHandle, uint8_t status) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; ee status =%d", fn, status);
...
//  if (eeHandle == ESE_HANDLE) mActiveeSEHandle = ESE_HANDLE;
  ...
  if (status == NFCEE_STATUS_ERROR) {
    LOG(ERROR) << StringPrintf("%s: EE ID %x unrecoverable error", fn, eeHandle & 0xFF);
    if (pEE) {
      pEE->ee_status = NFA_EE_STATUS_UNRESPONSIVE;

      if(((eeHandle & 0xFF) == 0x82) ||
              ((eeHandle & 0xFF) == 0x86)){
          LOG(ERROR)  << StringPrintf("%s: Disable/Enable NFC service following eSE unrecoverable error", fn);
...
void StSecureElement::nfaHciCallback(tNFA_HCI_EVT event,
  ...
  switch (event) {
    ...
    case NFA_HCI_ALLOCATE_GATE_EVT: {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_ALLOCATE_GATE_EVT; status=0x%X; gate=0x%X", fn,
    ...
    case NFA_HCI_OPEN_PIPE_EVT: {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_OPEN_PIPE_EVT; status=0x%X; pipe=0x%X", fn,
    ...
    case NFA_HCI_GET_REG_RSP_EVT:
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_GET_REG_RSP_EVT; status: 0x%X; pipe: 0x%X, len: %d",
      ...
      if (eventData->registry.pipe == sSecElem.mEseApduPipeId) {
        ...
        if (sSecElem.mEseApduGateId == ESE_ADPU_GATE_ID) {
          ...
          for (int g = 0; g < eventData->registry.data_len; g++) {
            LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: eventData->registry.reg_data[%d] = 0x%x", fn, g,
    ...
    case NFA_HCI_EVENT_RCVD_EVT:
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_EVENT_RCVD_EVT; code: 0x%X; pipe: 0x%X; data len: %u",
      ...
      if (eventData->rcvd_evt.evt_code == NFA_HCI_EVT_POST_DATA) {
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_EVENT_RCVD_EVT; NFA_HCI_EVT_POST_DATA", fn);
               ...
               NFA_HCI_EVT_HOT_PLUG)  // If HOT_PLUG
      {
        ...
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_EVENT_RCVD_EVT; EVT_WTX_REQUEST", fn);
        ...
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_EVENT_RCVD_EVT; data from APDU pipe", fn);
        SyncEventGuard guard(sSecElem.mTransceiveEvent);
      ...
      /* NCI 2.0 - Begin */
        ...
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_EVENT_RCVD_EVT; ATR from APDU pipe (rel12)", fn);

        sSecElem.mAtrInfo.length = eventData->rcvd_evt.evt_len;

        for (int g = 0; g < eventData->rcvd_evt.evt_len; g++) {
          LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: eventData->rcvd_evt.p_evt_buf[%d] = 0x%x", fn, g,
                eventData->rcvd_evt.p_evt_buf[g]);
    ...
    case NFA_HCI_HOST_LIST_EVT:  // HOST_LIST answer
    {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_HOST_LIST_EVT; status=0x%X; Number of hosts=0x%X", fn,
      ...
      if (hosts.status == NFA_STATUS_OK) {
        ...
        for (int i = 0; i < hosts.num_hosts; i++) {
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_HOST_LIST_EVT; host 0x%x in HOST_LIST", fn,
                hosts.host[i]);

          if (hosts.host[i] == UICC_ID) {
            sSecElem.mSeInfo.info[UICC_IDX].connected = true;
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_HCI_HOST_LIST_EVT; connected: 0x%x", fn,
                  sSecElem.mSeInfo.info[UICC_IDX].nfceeId);
...
bool StSecureElement::isBusy() {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("SecureElement::isBusy: %u", retval);
  return retval;
}
...
/* NCI 2.0 - End */
{
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; seID=0x%X enable = %d", fn, seID, enable);
  SeActivationLock();
  /* NCI 2.0 - Begin */
  ...
  /* NCI 2.0 - End */
  {
    ...
    /* NCI 2.0 - Begin */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: handle h=0x%X  id = 0x%X", fn,
    ...
    if (((eeItem.ee_handle & ~NFA_HANDLE_GROUP_EE) == seID)) {
      ...
      if (eeItem.ee_status == NFA_EE_STATUS_UNRESPONSIVE) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: seId 0x%X is unresponsive (not connected)", fn,
      ...
      if (((enable == true) && (eeItem.ee_status != NFA_EE_STATUS_ACTIVE)) ||
       ...
       // getEeInfo();
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: seId 0x%X already in the desired state", fn,
              (eeItem.ee_handle & ~NFA_HANDLE_GROUP_EE));
        break;
      }
    }
...
tNFA_STATUS StSecureElement::handleEnableSESeq(tNFA_EE_INFO* eeItem,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; ee_handle = 0x%X enable = %d", fn, eeItem->ee_handle,
        status);
  ...
  {  // Wait for NFCEE_MODE_SET_RSP
    ...
    /* NCI 2.0 - Begin */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Waiting for NFCEE_MODE_SET_RSP; h=0x%X", fn, eeItem->ee_handle);
    /* NCI 2.0 - End */
  ...
  if ((nfaStat == NFA_STATUS_OK) &&
       ...
       (eeItem->ee_status == NFC_NFCEE_STATUS_INACTIVE))) {
    {
      ...
      /* NCI 2.0 - Begin */
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Waiting for HCI HOT_PLUG; h=0x%X", fn, eeItem->ee_handle);
      /* NCI 2.0 - End */
      if (mHotPlugEvent.wait(HOT_PLUG_TIMER) == false)  // if timeout occurred
      {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: timeout waiting for HOT PLUG", fn);
      }
    }
    ...
    if (mSeActivationBitmap & HOT_PLUG_MASK) {
      ...
      if (mSeActivationBitmap & SE_INHIBITED_MASK) {
        /* NCI 2.0 - Begin */
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: SE is inhibited, waiting for pipe creations; h=0x%X", fn,
              eeItem->ee_handle);
        /* NCI 2.0 - End */
        if (mSeActivationEvent.wait(SE_ACTIVATION_TIMER) ==
            false)  // if timeout occurred
        {
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: timeout waiting for SE activation", fn);
          if (mSeActivationBitmap & SE_INITIALIZING) {
            mSeActivationBitmap &= ~SE_INITIALIZING;
            nfaStat = NFA_STATUS_FAILED;
          }
          ...
          if (mSeActivationBitmap & SE_INITIALIZING) {
            ...
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Activation started, wait more; h=0x%X", fn,
                  eeItem->ee_handle);
            if (mSeActivationEvent.wait(SE_ACTIVATION_TIMER) ==
                false)  // if timeout occurred
            {
              ...
              DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: still in initializing state, but have been triggered",
                    fn);
              mSeActivationBitmap &= ~SE_INITIALIZING;
            }
          }
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: No hot plug received", fn);
    }
  }
...
uint8_t StSecureElement::getActiveNfcee(uint8_t defaultNfceeId) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: checking id 0x%02X", fn,
                                                   defaultNfceeId);
  ...
  switch(defaultNfceeId){
    ...
    case 0x83:
    case 0x85:
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Looking for active UICC", fn);
      break;

    case 0x82:
    case 0x84:
    case 0x86:
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Looking for active eSE", fn);
      mask = 0x81;
      break;
...
uint8_t StSecureElement::getSENfceeId(uint8_t host_id) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: host_id: 0x%02X", fn,
                                                   host_id);

  if ((host_id == UICC_ID) ||
          ((host_id >= 0x80) && (host_id <= 0xBF))){
...
uint8_t StSecureElement::getConnectedNfceeId(uint8_t id){
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: requested id: 0x%02X, "
                    "corresponding connected nfceeId: 0x%02X", fn,
                                                     id, nciId);
