...
void NfcTag::isSkipMifare(bool status){
  mIsSkipMifare = status;
}
...
void NfcTag::discoverTechnologies(tNFA_ACTIVATED& activationData) {
  ...
  tNFC_ACTIVATE_DEVT& rfDetail = activationData.activate_ntf;
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << 
	StringPrintf("%s - mNumTechList =%d, mTechListIndex=%d", __func__, mNumTechList, mTechListIndex);
  ...
  if (NFC_PROTOCOL_T1T == rfDetail.protocol) {
    ...
    if ((NFC_DISCOVERY_TYPE_POLL_A == rfDetail.rf_tech_param.mode) ||
      ...
      if (fwi >= MIN_FWI && fwi <= MAX_FWI) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s - Setting the transceive timeout = %d, fwi = %0#x", fn, fwt, fwi);
    ...
    if (((rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_A) ||
        ...
        if((mIsSkipMifare) && (mTechParams[mNumTechList].param.pa.sel_rsp == 0x28)){
      ...
      if((mIsSkipMifare) && (mTechParams[mNumTechList].param.pa.sel_rsp == 0x28)){
          mTechParams[mNumTechList].param.pa.sel_rsp = 0x20;
               ...
               (rfDetail.rf_tech_param.mode ==
                NFC_DISCOVERY_TYPE_LISTEN_B_PRIME)) && mNumTechList < (MAX_NUM_TECHNOLOGY-1)) {
      mNumTechList++;
      mTechHandles[mNumTechList] = rfDetail.rf_disc_id;
      mTechLibNfcTypes[mNumTechList] = rfDetail.protocol;
      mTechList[mNumTechList] =
...
//    EXTNS_MfcInit(activationData);
    ...
    if ((NCI_PROTOCOL_UNKNOWN == rfDetail.protocol) &&
        ...
        mTechHandles [mNumTechList] = rfDetail.rf_disc_id;
        mTechLibNfcTypes [mNumTechList] = rfDetail.protocol;
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Tech type B, unknown ", fn);
      mTechList[mNumTechList] =
          TARGET_TYPE_ISO14443_3B;  // is TagTechnology.NFC_B by Java API
      // save the stack's data structure for interpretation later
      memcpy(&(mTechParams[mNumTechList]), &(rfDetail.rf_tech_param),
  ...
  for (int i=0; (i < mNumTechList) && (i < MAX_NUM_TECHNOLOGY); i++) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: index=%d; tech=0x%02X; handle=%d; nfc type=0x%02X", fn, i,
...
void NfcTag::discoverTechnologies(tNFA_DISC_RESULT& discoveryData) {
  ...
  if(discovery_ntf.protocol == NFC_PROTOCOL_T1T){
    LOG(ERROR) << StringPrintf("%s: Multitag detection contains T1T tag (no multitag support), skip", fn);
        goto TheEnd;
  }
  ...
  if (discovery_ntf.more != NCI_DISCOVER_NTF_MORE) {
    for (int i=0; (i < mNumTechList) && (i < MAX_NUM_TECHNOLOGY); i++) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: index=%d; tech=0x%02X; handle=%d; nfc type=0x%02X", fn, i, mTechList[i],
          mTechHandles[i], mTechLibNfcTypes[i]);
    }
  }
    mNumDiscTechList = mNumTechList;
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s; mNumDiscTechList=%x", fn, mNumDiscTechList);
...
void NfcTag::fillNativeNfcTagMembers5(JNIEnv* e, jclass tag_cls, jobject tag,
  ...
  if (NFC_DISCOVERY_TYPE_POLL_KOVIO == mTechParams[0].mode) {
    ...
    if ((NCI_PROTOCOL_UNKNOWN == mTechLibNfcTypes[0]) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Chinese Id Card", fn);
...
bool NfcTag::getP2pDetectedButPausedStatus(){
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: mIsP2pDetectedButPaused = %d",
                                                   fn, mIsP2pDetectedButPaused);
...
void NfcTag::selectTag(int idx) {
  ...
  if (foundIdx != -1) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf(
            "selectFirstTag: mTechHandles[foundIdx]=0x%X, mTechLibNfcTypes[foundIdx]=0x%x, "
            "rf_intf=0x%x, foundIdx=0x%x",
            mTechHandles[foundIdx], mTechLibNfcTypes[foundIdx], rf_intf, foundIdx);

    tNFA_STATUS stat = NFA_Select(mTechHandles[foundIdx],
        mTechLibNfcTypes[foundIdx], rf_intf);
    if (stat != NFA_STATUS_OK) {
...
void NfcTag::selectFirstTag() {
  ...
  if (foundIdx != -1) {
    if (mTechLibNfcTypes[foundIdx] == NFA_PROTOCOL_ISO_DEP) {
      ...
      rf_intf = NFA_INTERFACE_FRAME;
        DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("selectFirstTag: mTechHandles [foundIdx]=0x%X, mTechLibNfcTypes [foundIdx]=0x%x, rf_intf=0x%x, foundIdx=0x%x", 
			mTechHandles [foundIdx], mTechLibNfcTypes [foundIdx], rf_intf, foundIdx);
    tNFA_STATUS stat =
...
int NfcTag::checkNextValidProtocol(void)
{
  ...
  for (int i = selectedId; i < mNumDiscTechList; i++) {
        DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: nfa target idx=%d h=0x%X; protocol=0x%X",
...
void NfcTag::selectNextTag ()
{
    ...
    stat =
        NFA_Select(mTechHandles[foundIdx], mTechLibNfcTypes[foundIdx], rf_intf);
    if (stat == NFA_STATUS_OK) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: stat=%x; wait for activated ntf", fn,
                                 stat);
      {
        mWaitingForSelect = true;
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Starting timer", fn);
...
void NfcTag::connectionEventHandler(uint8_t event, tNFA_CONN_EVT_DATA* data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: event = 0x%02X", fn, event);
...
void NfcTag::selectCompleteStatus(bool status)
{
    ...
    {
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("selectCompleteStatus=%u", status);
...
**
*******************************************************************************/
bool NfcTag::isReselecting ()
{
    return mIsReselecting;
}
...
**
*******************************************************************************/
void NfcTag::setReselect (bool isReselecting)
{
    mIsReselecting = isReselecting;
}
