...
**
*******************************************************************************/
void StCardEmulationEmbedded::nfaEeCallback (tNFA_EE_EVT event, tNFA_EE_CBACK_DATA* eventData)
{
    static const char fn [] = "StCardEmulationEmbedded::nfaEeCallback";

    StCardEmulationEmbedded& StCardEmulationEmbedded = StCardEmulationEmbedded::getInstance();
    ...
    switch (event) {
        ...
        case NFA_EE_MODE_SET_EVT: {
          if(eventData->mode_set.ee_handle & 0x80){
            ...
            if(eventData->mode_set.status != NFA_STATUS_OK) {
                LOG(ERROR) << StringPrintf ("%s: NFA_EE_MODE_SET_EVT; status: 0x%04X  error !!!", fn, eventData->mode_set.status);
            }
        ...
        case NFA_EE_CONNECT_EVT: {
            ...
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: NFA_EE_CONNECT_EVT; status=%u, handle=0x%02X, interface=0x%02X", fn, eventData->connect.status,
                   eventData->connect.ee_handle, eventData->connect.ee_interface);

            if(eventData->connect.status == NFA_STATUS_OK) {
                sCardEmulationEmbedded.mIdConnectCee = eventData->connect.ee_handle & 0xFF;
            }
        ...
        case NFA_EE_DATA_EVT: {
            ...
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: NFA_EE_DATA_EVT; handle=0x%02X, length=0x%02X", fn, eventData->data.handle, eventData->data.len);
        ...
        case NFA_EE_DISCONNECT_EVT: {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: NFA_EE_DISCONNECT_EVT; handle=0x%02X", fn, eventData->data.handle);
            //mEeDisconnEvent
...
bool StCardEmulationEmbedded::enable(uint8_t ceeId, bool enable)
{
    ...
    /* NCI 2.0 - Begin */
    if(enable == true) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; Enabling CEE 0x%x", fn, ceeId);
    } else {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; Disabling CEE 0x%x", fn, ceeId);
    }
    /* NCI 2.0 - End */
    ...
    if ((nfaStat = NFA_EeGetInfo (&numEE, mEeInfo)) != NFA_STATUS_OK) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: found 0x%x EE", fn, numEE);

        for( i=0; i<numEE; i++) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: mEeInfo[i].ee_handle = 0x%x;", fn, mEeInfo[i].ee_handle);
            if(mEeInfo[i].ee_handle == (ceeId | NFA_HANDLE_GROUP_EE)) {
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: found matching entry for CEE ID 0x%x", fn, ceeId);
        ...
        if(found_ee) { //found
            ...
            if (((eeItem.ee_status != NFA_EE_STATUS_ACTIVE) && (enable == true)) ||
                ...
                //Updating routing table without NDEF AID: needs to be done while NDEF-NFCEE still active
                /* NCI 2.0 - Begin */
                if(enable == false) {
                    StRoutingManager::getInstance().removeAidRouting(NdefAid, sizeof(NdefAid));
...

                {
                    ...
                    if ((nfaStat = NFA_EeModeSet(eeItem.ee_handle, enable)) == NFA_STATUS_OK) {
                ...
                //Now wait NFCEE_MODE_SET_NTF
                {
                    ...
                    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Waiting for NFCEE_MODE_SET_NTF; h=0x%X", fn, eeItem.ee_handle);

                    if (mEeSetModeNtfEvent.wait(500) == false) { //wait for NFA_EE_MODE_SET_EVT
                        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: timeout waiting for NFCEE_MODE_SET_NTF", fn);
                        goto TheEnd;
                    }
                }
                /* NCI 2.0 - End */
                ...
                if(mNfaEECbStatus != NFA_STATUS_OK) {
                    LOG(ERROR) << StringPrintf("%s: Error in NCI_NFCEE_MODE_SET_RSP status (0x%x) - aborting procedure", fn, mNfaEECbStatus);
                    goto TheEnd;
                } else {
                    status = true;
                }
                ...
                /* NCI 2.0 - Begin */
                //Updating routing table with NDEF AID: Needs to be done when NDEF-NFCEE is active
                if(enable == true) {
                    StRoutingManager::getInstance().addAidRouting(NdefAid, sizeof(NdefAid), 0x10, 0x00);
                    StRoutingManager::getInstance().commitRouting();
                }
                /* NCI 2.0 - End */
            } else {
...
bool StCardEmulationEmbedded::connect(uint8_t ceeId)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; requesting to connect CEE 0x%x", fn, ceeId);
    ...
    if ((nfaStat = NFA_EeGetInfo (&numEE, mEeInfo)) != NFA_STATUS_OK) {
        ...
        if(found_ee) { //found
            ...
            if(android::isDiscoveryStarted()) {
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: RF state machine is currently in discovery mode - Deactivating", fn);
            ...
            if(mIdConnectCee != CEE_NO_APP) {
                LOG(ERROR) << StringPrintf ("%s: Already one CEE connect: 0x%x", fn, mIdConnectCee);
                goto TheEnd; //error
            }
            ...
            if (eeItem.ee_status == NFA_EE_STATUS_INACTIVE) {
                SyncEventGuard guard(mEeSetModeEvent);
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: set EE mode activate; h=0x%X", fn, eeItem.ee_handle);
                if ((nfaStat = NFA_EeModeSet(eeItem.ee_handle, true)) == NFA_STATUS_OK) {
                    mEeSetModeEvent.wait(); //wait for NFA_EE_MODE_SET_EVT
                } else {
                    LOG(ERROR) << StringPrintf("%s: NFA_EeModeSet failed; error=0x%X", fn, nfaStat);
                    goto TheEnd;
            ...
            //Create logical connection to NFCEE
            {
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: set EE mode activate; h=0x%X", fn, eeItem.ee_handle);
                if ((nfaStat = NFA_EeConnect(eeItem.ee_handle, eeItem.ee_interface[0], nfaEeCallback)) == NFA_STATUS_OK) {
                ...
                if(mNfaEECbStatus != NFA_STATUS_OK) {
                    LOG(ERROR) << StringPrintf("%s: Error in NCI_CORE_CONN_CREATE_RSP status (0x%x) - aborting procedure", fn, mNfaEECbStatus);
                    goto TheEnd;
                }
            }
...
bool StCardEmulationEmbedded::transceive (uint8_t ceeId, uint16_t tx_data_len,
        uint8_t*  tx_data, uint16_t& rx_data_size, uint8_t* rx_data)
{
    static const char fn [] = "StCardEmulationEmbedded::transceive";
    tNFA_STATUS nfaStat = NFA_STATUS_FAILED;

    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; NFCEE ID=%d; data len=%d;", fn, ceeId, tx_data_len);

    if(ceeId != mIdConnectCee) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; Requested NFCEE Id is not connected", fn);
        return false;
    }
    ...
    if(nfaStat != NFA_STATUS_OK) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; Error when calling NFA_EeSendData()", fn);
        return false;
    }
    ...
    if (mEeDataEvent.wait(500) == false) { //if timeout occurred
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: timeout waiting for answer to NFCEE data", fn);
        return false;
    }
...
bool StCardEmulationEmbedded::disconnect(uint8_t ceeId)
{
    ...
    if(ceeId != mIdConnectCee) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: enter; Requested NFCEE Id is already disconnected", fn);
        return false;
    }
    ...
    //Close the connection
    {
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: set EE mode activate; h=0x%X", fn, eeHandle);
        if ((nfaStat = NFA_EeDisconnect(eeHandle)) == NFA_STATUS_OK) {
            mEeDisconnEvent.wait(); //wait for NFA_EE_DISCONNECT_EVT
        } else {
            LOG(ERROR) << StringPrintf("%s: NFA_EeDisconnect failed; error=0x%X", fn, nfaStat);
            return false;
        }
    }
    //Disable the NFCEE
    {
        SyncEventGuard guard(mEeSetModeEvent);
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: set EE mode deactivate; h=0x%X", fn, ceeId);
