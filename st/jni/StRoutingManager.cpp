...
const JNINativeMethod StRoutingManager::sMethods[] = {
    ...
    {"doGetAidMatchingMode", "()I",
     (void*)StRoutingManager::com_android_nfc_cardemulation_doGetAidMatchingMode},
...
void StRoutingManager::forceRouting(uint8_t nfceeid) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s : nfceeid = 0x%X", fn, nfceeid);
  ...
  uint8_t activeNfceeId = StSecureElement::getInstance().getActiveNfcee(nfceeid);
...
void StRoutingManager::notifyDefaultRouteSet(int aidRoute, int mifareRoute, int isoDepRoute,
...
void StRoutingManager::updateDefaultProtocolRoute() {
  ...
  //Check if user has changed the defaukt routes through the NfcAdapterSettings APIs
  if(mUserDefaultIsoDepRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: - User modified ISO-DEP route = 0x%02X",
                        fn, mUserDefaultIsoDepRoute);
    isoDepRoute = mUserDefaultIsoDepRoute;
  }
  int connectedIsoDepRoute = StSecureElement::getInstance().getConnectedNfceeId(isoDepRoute);
...
int StRoutingManager::getScTypeFRouting(eJNI_ROUTING_TYPE type){
  ...
  int route, connectedRoute;

  //Get content of mEeInfo as it can change if a NTF is received during update of RT
  sEeInfoMutex.lock();
...
//      DLOG_IF(INFO, nfc_debug_enabled)
//          << StringPrintf("%s: User modified System Code route = 0x%02X",
//                          fn, mUserDefaultScRoute);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: type = %s, requested route = 0x%02X, "
      "connected route = 0x%02X, mIsSEFelicaCard = %d", fn,
    ...
    if(connectedRoute != NFC_DH_ID){
      for (uint8_t i = 0; i < localEeInfo.num_ee; i++) {
        ...
        if (eeHandle == (connectedRoute | NFA_HANDLE_GROUP_EE)) {
          ...
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s   EE[%u] Handle: 0x%04x  techF: 0x%02x",
              fn, i, eeHandle, localEeInfo.ee_disc_info[i].lf_protocol);

          //Target NFCEE supports type F
          if (localEeInfo.ee_disc_info[i].lf_protocol != 0){
            //If eSE and FELICA flag set OR
            //If UICC
            if((((connectedRoute == 0x82) ||
                (connectedRoute == 0x84) ||
                (connectedRoute == 0x86)) &&
                (mIsSEFelicaCard)) ||
                ((connectedRoute == 0x81) ||
                    ...
                    (connectedRoute == 0x85))){
              DLOG_IF(INFO, nfc_debug_enabled)
                  << StringPrintf("%s: found suitable route = 0x%02X", fn, connectedRoute);
...
void StRoutingManager::updateDefaultRoute() {
  ...
  if(mUserDefaultAidRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: User modified default AID route = 0x%02X",
                        fn, mUserDefaultAidRoute);
    aidRoute = mUserDefaultAidRoute;
  }
  int connectedDefaultEe = StSecureElement::getInstance().getConnectedNfceeId(aidRoute);
  ...
  if (scRoute != -1) {
    SyncEventGuard guard(mRoutingEvent);
    nfaStat = NFA_EeAddSystemCodeRouting(mDefaultSysCode, scRoute,
    ...
    if (nfaStat == NFA_STATUS_NOT_SUPPORTED) {
      ...
      mRoutingEvent.wait();
      DLOG_IF(INFO, nfc_debug_enabled) << fn
          << ": Succeed to register system code";
    } else {
    ...
    if (nfaStat == NFA_STATUS_NOT_SUPPORTED) {
      ...
      mRoutingEvent.wait();
      DLOG_IF(INFO, nfc_debug_enabled) << fn
      << ": Succeed to de-register system code";
    } else {
...
tNFA_TECHNOLOGY_MASK StRoutingManager::updateEeTechRouteSetting() {
  ...
  if(mUserDefaultOffHostRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: User modified default OffHost route = 0x%02X",
                        fn, mUserDefaultOffHostRoute);
...
void StRoutingManager::updateFullRoutes(){
  ...
  if(mUserDefaultAidRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User modified default AID route = 0x%02X", mUserDefaultAidRoute);
    aidRoute = mUserDefaultAidRoute;
  }
  int connectedDefaultEe = StSecureElement::getInstance().getConnectedNfceeId(aidRoute);
  ...
  if(mUserDefaultIsoDepRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User modified ISO-DEP route = 0x%02X", mUserDefaultIsoDepRoute);
    isoDepRoute = mUserDefaultIsoDepRoute;
  }
  int connectedIsoDepRoute = StSecureElement::getInstance().getConnectedNfceeId(isoDepRoute);

  DLOG_IF(INFO, nfc_debug_enabled)
      << fn << StringPrintf(" - Default ISO-DEP route = 0x%02X", connectedIsoDepRoute);
  ...
  if(mUserDefaultOffHostRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User modified OffHost route = 0x%02X", mUserDefaultOffHostRoute);
    offHostRoute = mUserDefaultOffHostRoute;
  }
  int connectedDefaultOffHostRoute =
                  StSecureElement::getInstance().getConnectedNfceeId(offHostRoute);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << fn << StringPrintf(" - Default Tech A/B route = 0x%02X", connectedDefaultOffHostRoute);
  ...
  if(mUserDefaultScRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User modified System Code route = 0x%02X", mUserDefaultScRoute);
  ...
  if(mUserDefaultFelicaRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User modified Felica route = 0x%02X", mUserDefaultFelicaRoute);
    felicaRoute = mUserDefaultFelicaRoute;
  }

  if(scRoute != felicaRoute){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - WARNING: default SC route and Felica route are different");
  }
  ...
  if(mUserDefaultMifareRoute != INVALID_ROUTE_VALUE){
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - User set a MIFARE route = 0x%02X", mUserDefaultMifareRoute);
    connectedMifareRoute =
            StSecureElement::getInstance().getConnectedNfceeId(mUserDefaultMifareRoute);
  }

  DLOG_IF(INFO, nfc_debug_enabled)
      << fn << StringPrintf(" - Default MIFARE route = 0x%02X", connectedMifareRoute);
...
//      if (connectedDefaultEe != connectedIsoDepRoute) {
//        DLOG_IF(INFO, nfc_debug_enabled)
//            << StringPrintf("%s: Default AID route was not set and is different from ISO-DEP route - "
//                    "connectedDefaultEe: 0x%02x", fn, connectedDefaultEe);
...
//      }
  ...
  for (uint8_t i = 0; i < localEeInfo.num_ee; i++) {
    ...
    if ((connectedMifareRoute != INVALID_ROUTE_VALUE) &&
      ...
      if ((localEeInfo.ee_disc_info[i].la_protocol & NFA_PROTOCOL_MASK_T2T) != 0){
        seTechMask |= NFA_TECHNOLOGY_MASK_A;
      }
    }
  ...
  /**************************************************************/
  {
    ...
    if ( (NfcStExtensions::HCE_TECH_MASK & NFA_TECHNOLOGY_MASK_A) &&
            ((mSeTechMask & NFA_TECHNOLOGY_MASK_A) == 0)) {
      nfaStat = NFA_EeSetDefaultTechRouting(NFC_DH_ID, techMask, 0, 0,
                                            mSecureNfcEnabled ? 0 : techMask,
                                            0,
                                            0);
      if (nfaStat == NFA_STATUS_OK) mRoutingEvent.wait();
      else LOG(ERROR) << fn << "Fail to set default tech routing for Nfc-A";
    }
    ...
    if ( (NfcStExtensions::HCE_TECH_MASK & NFA_TECHNOLOGY_MASK_B) &&
      ...
      nfaStat = NFA_EeSetDefaultTechRouting(NFC_DH_ID, techMask, 0, 0,
                                            ...
                                            0);
      if (nfaStat == NFA_STATUS_OK) mRoutingEvent.wait();
      else LOG(ERROR) << fn << "Fail to set default tech routing for Nfc-B";
    }
    ...
    if (felicaRoute ==0){
      nfaStat = NFA_EeSetDefaultTechRouting(NFC_DH_ID, techMask, 0, 0,
                                            ...
                                            0);
      if (nfaStat == NFA_STATUS_OK) mRoutingEvent.wait();
      else LOG(ERROR) << fn << "Fail to set default tech routing for Nfc-F";
    }
  }
...
static void reconfLmrtNoTechCb(union sigval){
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s: timer elapsed, "
      "reconfiguring routing table with all tech removed", fn);
  ...
  switch (event) {
    ...
    case NFA_EE_STATUS_NTF_EVT: {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            ...
            eventData->status_ntf.status, ((tNFA_EE_STATUS_NTF*)eventData)->nfcee_id);
    ...
    case NFA_EE_POWER_CTRL_EVT: {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFA_EE_POWER_CTRL_EVT; status: 0x%04X  nfcee_id: 0x%04X", fn,
            eventData->power_ctrl_rsp.status,
            eventData->power_ctrl_rsp.nfcee_id);
    ...
    case NFA_EE_DISCOVER_REQ_EVT: {
      ...
      for(int i=0; i< eventData->discover_req.num_ee; i++){
        if((eventData->discover_req.ee_disc_info[i].la_protocol == 0) &&
          ...
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_EE_DISCOVER_REQ_EVT; All techs removed for NFCEE 0x%02X, "
              "wait 500ms for reconfiguration", __func__,
              eventData->discover_req.ee_disc_info[i].ee_handle);
    ...
    case NFA_EE_UPDATED_EVT: {
      ...
      routingManager.notifyDefaultRouteSet(routingManager.mConnectedDefaultAidRoute,
...
int StRoutingManager::registerT3tIdentifier(uint8_t* t3tId, uint8_t t3tIdLen) {
  ...
  if (mIsScbrSupported) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Succeed to register system code on DH , systemCode = %X", fn, systemCode);
...
void StRoutingManager::setUserDefaultRoutesPref(int mifareRoute,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: mifareRoute: 0x%02X, isoDepRoute: 0x%02X, "
              "felicaRoute: 0x%02X, abTechRoute: 0x%02X, scRoute: 0x%02X, "
              "aidRoute: 0x%02X",
                      fn, mifareRoute, isoDepRoute, felicaRoute, abTechRoute,
                      scRoute, aidRoute);
...
**
*******************************************************************************/
int StRoutingManager::com_android_nfc_cardemulation_doGetDefaultRouteDestination(
    JNIEnv*) {
...
bool StRoutingManager::clearAidTable() {
  ...
  if(mDeinitializing){
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: De-initializing, exit", fn);
    return true;
  }
...
void StRoutingManager::setSEFelicaCardEnable(bool status){
