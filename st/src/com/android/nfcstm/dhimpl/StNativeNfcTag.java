...
public class StNativeNfcTag implements TagEndpoint {
    ...
    native boolean doEnableLptdPresenceCheck(boolean enable);

    @Override
    public synchronized boolean enableLptdPresenceCheck(boolean enable) {
        if (mWatchdog != null) {
            mWatchdog.pause();
        }
        boolean result = doEnableLptdPresenceCheck(enable);
        if (mWatchdog != null) {
            mWatchdog.doResume();
        }
        return result;
