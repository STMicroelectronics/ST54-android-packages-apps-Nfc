...
import com.android.nfcstm.cardemulation.EnabledNfcFServices;
import com.android.nfcstm.cardemulation.RegisteredNfcFServicesCache;
import com.android.nfcstm.cardemulation.RegisteredT3tIdentifiersCache;
...
public class EnableNfcFServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        when(ForegroundUtils.getInstance(mockContext.getSystemService(ActivityManager.class)))
                .thenReturn(mForegroundUtils);
        RegisteredNfcFServicesCache registeredNfcFServicesCache =
                mock(RegisteredNfcFServicesCache.class);
        mComponentName = mock(ComponentName.class);
        mNfcFServiceInfo = mock(NfcFServiceInfo.class);
        when(registeredNfcFServicesCache.getService(1, mComponentName))
                .thenReturn(mNfcFServiceInfo);
        RegisteredT3tIdentifiersCache registeredT3tIdentifiersCache =
                mock(RegisteredT3tIdentifiersCache.class);
                ...
                .runOnMainSync(
                        () ->
                                mEnabledNfcFServices =
                                        new EnabledNfcFServices(
                                                mockContext,
                                                registeredNfcFServicesCache,
                                                registeredT3tIdentifiersCache,
                                                (userId, service) -> {
                                                    Log.d(
                                                            TAG,
                                                            "CallBack is Received for userid "
                                                                    + userId);
                                                }));
    ...
    public void testRegisterEnabledForegroundService() {
        ...
        when(mForegroundUtils.registerUidToBackgroundCallback(mEnabledNfcFServices, 1))
                .thenReturn(true);
        boolean isRegistered =
                mEnabledNfcFServices.registerEnabledForegroundService(mComponentName, 1);
