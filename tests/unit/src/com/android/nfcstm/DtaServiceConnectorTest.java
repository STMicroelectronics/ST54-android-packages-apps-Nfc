...
public class DtaServiceConnectorTest {
    ...
    public void setUp() throws Exception {
        ...
        when(mockResources.getBoolean(eq(R.bool.tag_intent_app_pref_supported))).thenReturn(false);
        ...
        mockContext =
                new ContextWrapper(context) {
                    ...
                    public PackageManager getPackageManager() {
                        ...
                        resolveInfo.serviceInfo.packageName = "com.android.nfcstm";
                        resolveInfo.serviceInfo.name = "Nfc";
                        List<ResolveInfo> resolveInfos = new ArrayList<>();
                        resolveInfos.add(resolveInfo);
                        when(implicitIntent.getAction()).thenReturn("nfcaction");
                        when(packageManager.queryIntentServices(implicitIntent, 0))
        ...
        InstrumentationRegistry.getInstrumentation()
                .runOnMainSync(() -> mDtaServiceConnector = new DtaServiceConnector(mockContext));
    ...
    public void testCreateExplicitFromImplicitIntent() {
        ...
        Intent intent =
                DtaServiceConnector.createExplicitFromImplicitIntent(mockContext, implicitIntent);
        Assert.assertNotNull(intent);
        ComponentName componentName = intent.getComponent();
        Assert.assertNotNull(componentName);
        Assert.assertEquals("com.android.nfcstm", componentName.getPackageName());
    }
}
