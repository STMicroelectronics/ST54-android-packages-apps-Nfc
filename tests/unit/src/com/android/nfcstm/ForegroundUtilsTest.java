...
public class ForegroundUtilsTest {
    ...
    public void setUp() throws Exception {
        ...
        InstrumentationRegistry.getInstrumentation()
                .runOnMainSync(() -> mForegroundUtils = new ForegroundUtils(mActivityManager));
    ...
    public void testRegisterUidToBackgroundCallback() {
        ...
        ForegroundUtils.Callback callback =
                uid -> {
                    Log.d(TAG, "testRegisterUidToBackgroundCallback callback received");
                };
        when(mActivityManager.getUidImportance(0))
    ...
    public void testOnUidImportance() {
        ...
        mForegroundUtils.onUidImportance(
                0, ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
        List<Integer> uids = mForegroundUtils.getForegroundUids();
        Assert.assertNotNull(uids);
        Assert.assertTrue(uids.size() > 0);
        mForegroundUtils.onUidImportance(0, ActivityManager.RunningAppProcessInfo.IMPORTANCE_GONE);
    ...
    public void testOnUidImportanceBackground() {
        ...
        SparseArray<List<ForegroundUtils.Callback>> backGroundCallbacks =
                mForegroundUtils.getBackgroundCallbacks();
        ...
        mForegroundUtils.onUidImportance(
                0, ActivityManager.RunningAppProcessInfo.IMPORTANCE_BACKGROUND);
    ...
    public void testGetForegroundUids() {
        ...
        mForegroundUtils.onUidImportance(
                0, ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
        mForegroundUtils.onUidImportance(
                1, ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
        mForegroundUtils.onUidImportance(
                2, ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND);
        List<Integer> uids = mForegroundUtils.getForegroundUids();
        Assert.assertNotNull(uids);
        int uid = uids.get(0);
