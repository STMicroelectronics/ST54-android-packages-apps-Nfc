...
import static com.android.nfcstm.NfcWifiProtectedSetup.NFC_TOKEN_MIME_TYPE;
...
public class NfcWifiProtectedSetupTest extends TestCase {
    ...
    private NdefRecord createWifiRecord(String[] data) {
        ...
        byte[] ssidLength = {
            (byte) ((int) Math.floor(ssid.length() / 256)), (byte) (ssid.length() % 256)
        };
        byte[] passwordLength = {
            (byte) ((int) Math.floor(password.length() / 256)), (byte) (password.length() % 256)
        };
        byte[] cred = {0x00, 0x36};
