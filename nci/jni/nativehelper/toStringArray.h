...
jobjectArray toStringArray(JNIEnv* env, size_t count, StringVisitor&& visitor) {
  ...
  ScopedLocalRef<jobjectArray> result(
      env, env->NewObjectArray(count, stringClass, NULL));
...
inline jobjectArray toStringArray(JNIEnv* env,
  ...
  return toStringArray(env, strings.size(),
                       [&strings](size_t i) { return strings[i].c_str(); });
}
...
jobjectArray toStringArray(JNIEnv* env, Counter* counter, Getter* getter) {
  return toStringArray(env, counter(),
                       [getter](size_t i) { return getter(i); });
}
