...
public class HostEmulationManager {
    ...
    static final String NFC_PACKAGE = "com.android.nfcstm";
    ...
    HostEmulationManager(
            Context context, Looper looper, RegisteredAidCache aidCache, StatsdUtils statsdUtils) {
        mContext = context;
        mLooper = looper;
        ...
        mHandler.post(
                () -> {
                    if (DBG && (service != null))
                        ...
                        Log.d(TAG, "onPreferredPaymentServiceChanged() - service: " + "null");
                    synchronized (mLock) {
    ...
    private Pair<Messenger, ComponentName> getForegroundServiceAndNameOrDefault() {
        ...
        int preferredServiceUserId = preferredService.first != null ? preferredService.first : -1;
    ...
    class AutoDisableObserveModeRunnable implements Runnable {
        ...
        public void run() {
            synchronized (mLock) {
                ...
                Log.w(
                        TAG,
                        "Observe mode not disabled and no application from the following "
        ...
        synchronized (mLock) {
            ...
            if (mPollingLoopState == PollingLoopState.DELIVERING_TO_PREFERRED) {
                ...
                if (serviceAndName != null) {
                    ...
                    Log.i(
                            TAG,
                            "No preferred service to deliver polling frames to,"
        ...
        if (DBG && (service != null))
            Log.d(
                    ...
                    "onPreferredForegroundServiceChanged() - service: "
                            + service.flattenToString());
        else if (DBG) Log.d(TAG, "onPreferredForegroundServiceChanged() - service: " + "null");
    ...
    Messenger bindServiceIfNeededLocked(int userId, ComponentName service) {
        ...
        int preferredPaymentUserId =
                preferredPaymentService.first != null ? preferredPaymentService.first : -1;
        ...
        if (mPaymentServiceName != null
            ...
            Log.d(
                    TAG,
                    "bindServiceIfNeededLocked() - Service should be bound as payment service but"
                            + " is not, binding now");
    ...
    void bindPaymentServiceLocked(int userId, ComponentName service) {
        if (DBG && (service != null))
            Log.d(TAG, "bindPaymentServiceLocked() - service: " + service.flattenToString());
        else if (DBG) Log.d(TAG, "bindPaymentServiceLocked() - service: " + "null");
        ...
        try {
            if (mContext.bindServiceAsUser(
                ...
                Log.e(
                        TAG,
                        "bindPaymentServiceLocked() - Could not bind (persistent) payment"
                                + " service.");
            }
        } catch (SecurityException e) {
            Log.e(
                    TAG,
                    "bindPaymentServiceLocked() - Could not bind payment service due to security"
                            + " exception.");
        }
    }
    ...
    void unbindServiceIfNeededLocked() {
        if (mServiceBound) {
            Log.d(TAG, "unbindServiceIfNeededLocked() - Unbinding from service " + mServiceName);
            try {
                mContext.unbindService(mConnection);
            } catch (Exception e) {
                Log.d(
                        TAG,
                        "unbindServiceIfNeededLocked() - Failed to unbind service " + mServiceName,
                        e);
            }
            mServiceBound = false;
        }
    ...
    String findSelectAid(byte[] data) {
        ...
        if (data[0] == 0x00 && data[1] == INSTR_SELECT && data[2] == 0x04) {
            if (data[3] != 0x00) {
                Log.d(
                        TAG,
                        "findSelectAid() - Selecting next, last or previous AID occurrence is not"
                                + " supported");
            }
    ...
    private ServiceConnection mPaymentConnection =
            new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    synchronized (mLock) {
                        ...
                        if (!mLastBoundPaymentServiceName.equals(name)) {
                            Log.i(
                                    TAG,
                                    "mPaymentConnection - onServiceConnected() - Ignoring bound"
                                            + " payment service, "
                                            + name
                                            + " != "
                        ...
                        Log.i(
                                TAG,
                                "mPaymentConnection - onServiceConnected() - Payment service bound:"
                                        + " "
                                        + name);
                    }
                }
                ...
                public void onServiceDisconnected(ComponentName name) {
                    Log.i(
                            TAG,
                            "mPaymentConnection - onServiceDisconnected() - Payment service"
                                    + " disconnected: "
                                    + name);
                    synchronized (mLock) {
                        mPaymentService = null;
            ...
            new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    synchronized (mLock) {
                        ...
                        Log.i(
                                TAG,
                                "mConnection - onServiceConnected() - Payment service connected: "
                                        + name);
                        ...
                        if (mSelectApdu != null) {
                            ...
                            mSelectApdu = null;
                        } else if (mPollingFramesToSend != null
                                && mPollingFramesToSend.containsKey(name)) {
                            sendPollingFramesToServiceLocked(
                                    mService, mPollingFramesToSend.get(name));
                            ...
                            Log.d(
                                    TAG,
                                    "mConnection - onServiceConnected() - bound with nothing to"
                                            + " send");
                        }
                    }
                }
                ...
                public void onServiceDisconnected(ComponentName name) {
                    if (DBG && (name != null))
                        ...
                        Log.d(TAG, "mConnection - onServiceDisconnected() - name: " + "null");
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == HostApduService.MSG_RESPONSE_APDU) {
                ...
                if (data == null || data.length == 0) {
                    Log.e(TAG, "MessageHandler - handleMessage() - Dropping empty R-APDU");
                    return;
                }
                int state;
                synchronized (mLock) {
                ...
                if (state == STATE_XFER) {
                    Log.d(TAG, "MessageHandler - handleMessage() - Sending data");
                    ...
                    Log.d(
                            TAG,
                            "MessageHandler - handleMessage() - Dropping data, wrong state "
                                    + Integer.toString(state));
                }
            } else if (msg.what == HostApduService.MSG_UNHANDLED) {
                synchronized (mLock) {
                    Log.d(TAG, "MessageHandler - handleMessage(MSG_UNHANDLED)");
