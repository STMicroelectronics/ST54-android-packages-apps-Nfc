...
import com.android.nfc.st.SecureElementSelector;
...
public class NfcAddonWrapper implements ISeController.Callback {
    ...
    private NfcAddonWrapper(Context context, StNativeNfcManager manager,
        ...
        mSecureElementSelector = new SecureElementSelector(
                mContext, mNativeNfcManager, mStExtensions, NfcService.getInstance(), this);


    }

    public static void createSingleton(Context context, StNativeNfcManager manager,
            NativeNfcStExtensions stExtensions) {
...
//            ContentResolver mContentResolver = mContext.getContentResolver();
        ...
        if ((mPrefs.getInt(PREF_MODE_P2P, NfcSettingsAdapter.FLAG_ON)
                == NfcSettingsAdapter.FLAG_ON) && (isBeamCapable)) {
    ...
    public void setModeFlag(boolean isNfcEnabled, int mode, int flag, Object syncObj) {
        Log.d(TAG, "setModeFlag() - isNfcEnabled = " + isNfcEnabled + ", mode = "
                + mode + ", flag = " + flag + ", syncObj = " + syncObj);
        ...
        synchronized (syncObj) {
            if ((mode > (NfcSettingsAdapter.MODE_READER | NfcSettingsAdapter.MODE_P2P
                    | NfcSettingsAdapter.MODE_HCE) || mode < 0) ||
                    (flag != NfcSettingsAdapter.FLAG_ON && flag != NfcSettingsAdapter.FLAG_OFF)) {
                Log.d(TAG, "setModeFlag() - incorrect mode:" + mode + " or flag:" + flag + ", return");
                return;
            }
