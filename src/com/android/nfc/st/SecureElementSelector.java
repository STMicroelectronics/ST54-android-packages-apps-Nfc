...
import com.android.nfc.st.*;
import com.android.nfc.st.NfcSimStateObserver.SimEventListener;
...
public class SecureElementSelector
    ...
    static private final String TAG = "Nfc_SecureElementSelector";
    ...
    public void onSimStateChanged(int simId, int newPresenceState) {
        ...
        boolean changed = false;
        Log.d(TAG, "onSimStateChanged() - simId = " + simId + " newPresenceState = " + newPresenceState +
                " mUICC1State: " + getSim1State() + " mUICC2State: " + getSim2State() +
                " airplane: " + NfcSimStateObserver.getInstance().isAirplaneModeHidingSimPresence());
        ...
        if ((simId != NfcSimStateObserver.SimEventListener.SIM1) && (simId != NfcSimStateObserver.SimEventListener.SIM2)) {
        ...
        if ( (init == true)
            ...
            if (newPresenceState == NfcSimStateObserver.SimEventListener.ABSENT) {
                ...
                int wanted = getUserDesiredUiccId();
                if ((wanted == currentId)
                        && ( ! currentState.equals(SE_STATE_ACTIVATED))) {
                    // The SIM selected by user has been inserted, activate it (if no other element is active).
        ...
        if (newPresenceState != NfcSimStateObserver.SimEventListener.ABSENT) {
            // Don t fallback when SIM is removed as we are notified of the two removals in sequence.
...
//            SetForceRoutingIfNeeded();
    ...
    public SecureElementSelector(Context context, StNativeNfcManager nativeNfcManager,
            NativeNfcStExtensions stExtensions,
            Object syncLock, ISeController.Callback callback) {
        mContext = context;
        mCallback = callback;
    ...
    private boolean restorePreviousSe() {
        ...
        if (UiccStr == null && eSEStr == null ) {
            ...
            Log.d(TAG, "restorePreviousSe() - no previous record, default '" + DefaultSe +
                    "', set UICC = " + UiccStr +", eSE = " + eSEStr);
        ...
        Log.d(TAG, "restorePreviousSe() - DesiredUicc = " + String.format("0x%02X", DesiredUicc) +
                "- DesiredeSE = " + String.format("0x%02X", DesiredeSE));
        boolean ret = false;
        if ((DesiredUicc != 0x0) && ((simPresenceStableForSwp(mSim1Presence) && simPresenceStableForSwp(mSim2Presence)) ||
                NfcSimStateObserver.getInstance().isAirplaneModeHidingSimPresence())) {
    ...
    private void onActiveUiccChanged(int SeID, boolean storePreference) {
        Log.d(TAG, "onActiveUiccChanged(" + storePreference + ") - SeID = " + Integer.toHexString(SeID));
        synchronized (mSyncLock) {
            {
                int ActSe, ActSeNew;
                int errors = 0;
                ...
                if (SeID == 0) {
                    for (Map.Entry<Integer, String> entry : SeList.entrySet()) {
                        Log.d(TAG, "onActiveUiccChanged() - SeID = " + String.format("0x%02X", entry.getKey())
                                + " Status = " + entry.getValue());
                        if ((entry.getValue().equals(SE_STATE_ACTIVATED))
                                && (entry.getKey()!= 0x82) && (entry.getKey()!= 0x84)
                                && (entry.getKey()!= 0x86)) {
                            if (false == mStExtensions.EnableSE(entry.getKey(), false))
                                errors++;
                        }
                    }
                    ...
                    Log.d(TAG,
                            "onActiveUiccChanged() - Disable " + Integer.toHexString(ActSe)
                            +
                            " Activate " + Integer.toHexString(SeID) + " ("
                            + SeList.get(SeID) + ")");

                    if ( ! SE_STATE_NOT_AVAILABLE.equals(SeList.get(SeID))) {
                        if (ActSe != 0) {
                            if (false == mStExtensions.EnableSE(ActSe,
                                    false)) { // Disable the activated SE
                                mNativeNfcManager.NfceeDiscover();
                                RefreshSeStatus(true);
                            }
                        }
                        if (false == mStExtensions.EnableSE(SeID,
                                true)) { //Enable the desired SE
                            Log.d(TAG, "onActiveUiccChanged() - failed to activate "
...
//                    SetForceRoutingIfNeeded();
                ...
                if (ActSeNew == SeID) {
                    Log.d(TAG, "onActiveUiccChanged() - ActSeNew = " + String.format("0x%02X",
                            ActSeNew));
                } else {
...
/*    public void SetForceRoutingIfNeeded() {
    ...
    public boolean EnableSecureElement (String str, boolean enable)
    {
        ...
        if (DBG)
          Log.d(TAG, "EnableSecureElement() - Id: " + str + ", enable: " + enable + ", targetSE = " + String.format("0x%02X", targetSE));
        ...
        if (str.equals(SETTING_STR_ESE) || str.equals(SETTING_STR_DHSE)) {
            ...
            if(enable == false && mStActiveeSe == targetSE) {
                ...
                if (mStActiveeSe != 0)
                    mStExtensions.EnableSE(mStActiveeSe, false);
    ...
    private void RefreshSeStatus(boolean initial) {
        ...
        Log.d(TAG,
                "RefreshSeStatus(start, " + initial + ") - " + "nfceeid[0]: " + String.format("0x%02X", nfceeid[0])
                + " conInfo[0]: " + String.format("0x%02X", conInfo[0]) + ", nfceeid[1]: "
                + String.format("0x%02X", nfceeid[1]) + " conInfo[1]: " + String.format(
                        "0x%02X", conInfo[1]));

        if (!NfcSimStateObserver.getInstance().isAirplaneModeHidingSimPresence()) {
            Log.d(TAG,
                    "RefreshSeStatus() - not in Airplane mode, so using presence information SIM1="
                            + String.format("0x%02X", mSim1Presence) + ", SIM2=" + String.format(
                                    "0x%02X", mSim2Presence));
        }
        ...
        Log.d(TAG, "RefreshSeStatus() - mStActiveeSe = " + String.format("0x%02X", mStActiveeSe) + " mStActiveUicc = " + String.format("0x%02X", mStActiveUicc));

        Log.d(TAG, "RefreshSeStatus(end) - mUICC1State: " + getSim1State()
                + ", mUICC2State: " + getSim2State() + ", meSEState: "
                + getESeState() + ", mDHSEState: " + getDHSEState());
    }
    ...
    public List<String> getSecureElementsStatus() {
        ...
        s = SeList.get(0x81);
        if (s != null) { ret.add(SETTING_STR_SIM1 + ":" + s); }

        // then SIM2 if any
        s = SeList.get(0x83);
        if (s != null) { ret.add(SETTING_STR_SIM2 + ":" + s); }
        s = SeList.get(0x85);
        if (s != null) { ret.add(SETTING_STR_SIM2 + ":" + s); }

        // finally ESE. As it is not removable, only show when at least available
        s = SeList.get(0x82);
        if (s != null && ! SE_STATE_NOT_AVAILABLE.equals(s)) { ret.add(SETTING_STR_ESE + ":" + s); }
        s = SeList.get(0x86);
        if (s != null && ! SE_STATE_NOT_AVAILABLE.equals(s)) { ret.add(SETTING_STR_ESE + ":" + s); }
        s = SeList.get(0x84);
        if (s != null && ! SE_STATE_NOT_AVAILABLE.equals(s)) { ret.add(SETTING_STR_DHSE + ":" + s); }
    ...
    private class MyContentProviderUtil {
        ...
        public String getUserDesiredUiccString() {
            if (mPrefs.contains(PREF_UICC1_ACTIVATION_STATUS) ||
                ...
                if(mPrefs.getBoolean(PREF_UICC1_ACTIVATION_STATUS , false))
                    return SETTING_STR_SIM1;
                else if (mPrefs.getBoolean(PREF_UICC2_ACTIVATION_STATUS , false))
                    return SETTING_STR_SIM2;
                else
                    return SETTING_STR_OFF;
            } else {
                return null;
            }
        }
        ...
        public String getUserDesiredeSEString() {
            if (mPrefs.contains(PREF_ESE_ACTIVATION_STATUS) ||
                ...
                if(mPrefs.getBoolean(PREF_ESE_ACTIVATION_STATUS , false))
                    return SETTING_STR_ESE;
