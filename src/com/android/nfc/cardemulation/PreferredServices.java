...
public class PreferredServices implements com.android.nfc.ForegroundUtils.Callback {
    ...
    void loadDefaultsFromSettings(int userId) {
        ...
        if (DBG)
            Log.d(TAG, "loadDefaultsFromSettings() - new defaultNfcPayment name: " + name);
        if (mPaymentDefaults.currentPreferred != null) {
            if (DBG)
                Log.d(TAG, "loadDefaultsFromSettings() - mPaymentDefaults.currentPreferred: "
        ...
        try {
            ...
            if (DBG) Log.d(TAG,
                    "loadDefaultsFromSettings() - current preferForeground: " + preferForeground);
        ...
        if (paymentDefaultChanged) {
            ...
            if (newDefault != null) {
                ...
                if (service != null) {
                    ...
                    if (DBG)
                        Log.d(TAG, "loadDefaultsFromSettings() - new default payment description: "
                                + description);
                }
            }
    ...
    void computePreferredForegroundService() {
        ...
        if (mForegroundCurrent != null) {
            if (DBG) Log.d(TAG, "computePreferredForegroundService() - mForegroundCurrent: "
    ...
    public void overflowNotificationClosed(boolean canceled) {
        if (DBG) Log.d(TAG, "overflowNotificationClosed(" + canceled + "," + mIsPendingOverflowResolution + ")");
    ...
    public void onServicesUpdated() {
        ...
        if(mIsPendingOverflowResolution == true){
            ...
            if(NfcService.getInstance().getAidRoutingTableStatus() == true){
                ...
                if (DBG) Log.d(TAG, "onServicesUpdated() - Pending Overflow Resolution - not yet solved");

            } else {
            if (DBG) Log.d(TAG, "onServicesUpdated() - Pending Overflow Resolution - final step");
            mIsPendingOverflowResolution = false;
        ...
        synchronized (mLock) {
            ...
            if (mForegroundCurrent != null) {
                ...
                if (DBG) Log.d(TAG, "onServicesUpdated() - mForegroundCurrent : "
                        + mForegroundCurrent.flattenToString());
    ...
    boolean isForegroundAllowedLocked(ComponentName service) {
        ...
        if (service != null) {
            ...
            if (DBG)
                Log.d(TAG, "isForegroundAllowedLocked() - service: null");
        }
        ...
        StApduServiceInfo serviceInfo = mServiceCache.getStService(ActivityManager.getCurrentUser(),
                service);
        if (serviceInfo == null) {
            Log.d(TAG,
        ...
        if (!mPaymentDefaults.preferForeground) {
            ...
            StApduServiceInfo paymentServiceInfo = mServiceCache.getStService(
            ...
            if (paymentServiceInfo != null && otherAids != null && otherAids.size() > 0) {
                for (String aid : otherAids) {
                    ...
                    if (CardEmulation.CATEGORY_PAYMENT.equals(resolveInfo.category) &&
                        ...
                        if(DBG2) Log.d(TAG, "AID " + aid + " is handled by the default payment app, " +
    ...
    public boolean registerPreferredForegroundService(ComponentName service, int callingUid) {
        ...
        if (service != null) {
            if (DBG) Log.d(TAG,
                    "registerPreferredForegroundService() - service: " + service.flattenToString());
        } else {
        ...
        if (success) {
            ...
            StApduServiceInfo apduService = mServiceCache.getStService(
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println("HCENfc_PreferredServices -     *** Current preferred foreground service: "
                + mForegroundCurrent);
        pw.println("HCENfc_PreferredServices -     *** Current preferred payment service: "
                + mPaymentDefaults.currentPreferred);
        pw.println("HCENfc_PreferredServices -         Next tap default: " + mNextTapDefault);
        pw.println("HCENfc_PreferredServices -         Default for foreground app (UID: "
                + mForegroundUid +
                "): " + mForegroundRequested);
        pw.println("HCENfc_PreferredServices -         Default in payment settings: "
                + mPaymentDefaults.settingsDefault);
        pw.println("HCENfc_PreferredServices -         Payment settings allows override: "
