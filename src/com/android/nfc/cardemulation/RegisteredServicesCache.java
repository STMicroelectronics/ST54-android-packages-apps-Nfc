...
public class RegisteredServicesCache {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (uid != -1) {
                    ...
                    if (!replaced) {
                        ...
                        if (DBG) Log.d(TAG, "Ignoring package intent due to package being replaced.");
                    }
                }
            }
        };
    ...
    boolean containsStServiceLocked(ArrayList<StApduServiceInfo> services, ComponentName serviceName) {
    ...
    ArrayList<StApduServiceInfo> getInstalledStServices(int userId) {
        ...
        if (DBG) Log.d(TAG,
                "getInstalledStServices() - found " + resolvedServices.size() + " resolvedServices");
        ...
        for (ResolveInfo resolvedService : resolvedServices) {
            try {
                ...
                if (DBG) Log.d(TAG, "getInstalledStServices() - componentName:  "
                ...
                if (pm.checkPermission(android.Manifest.permission.NFC, si.packageName) !=
                    ...
                    Log.e(TAG, "getInstalledStServices() - Skipping application component "
                            + componentName +
                ...
                if (!android.Manifest.permission.BIND_NFC_SERVICE.equals(
                        si.permission)) {
                    Log.e(TAG, "getInstalledStServices() - Skipping APDU service " + componentName +
                ...
                Log.w(TAG, "getInstalledStServices() - Unable to load component info "
    ...
    public void invalidateCache(int userId) {
        ...
        synchronized (mLock) {
            ...
            for (StApduServiceInfo service : validServices) {
                if (DBG2) Log.d(TAG,
                        "invalidateCache() - Adding service: " + service.getComponent() +
    ...
    private void readDynamicSettingsLocked() {
        ...
        try {
                    ...
                    if (eventType == XmlPullParser.START_TAG) {
                        ...
                        if ("aid-group".equals(tagName) && parser.getDepth() == 3 && inService) {
                            ...
                            if (group != null) {
                                ...
                                Log.e(TAG, "readDynamicSettingsLocked() - Could not parse AID group.");
                            }
                        }
            ...
            Log.e(TAG, "readDynamicSettingsLocked() - Could not parse dynamic AIDs file, trashing.");
    ...
    public boolean setOffHostSecureElement(int userId, int uid, ComponentName componentName,
        ...
        if (DBG) Log.d(TAG, "setOffHostSecureElement() - componentName: " + componentName.flattenToString() +
            " offHostSE: " + offHostSE);
    ...
    public boolean unsetOffHostSecureElement(int userId, int uid, ComponentName componentName) {
        ...
        synchronized (mLock) {
            ...
            StApduServiceInfo serviceInfo = getStService(userId, componentName);
            if (serviceInfo == null) {
                Log.e(TAG, "unsetOffHostSecureElement() - Service " + componentName + " does not exist.");
                return false;
            }
            if (serviceInfo.getUid() != uid) {
                // This is probably a good indication something is wrong here.
                ...
                while (eventType != XmlPullParser.END_DOCUMENT) {
                    ...
                    if (eventType == XmlPullParser.START_TAG) {
                        ...
                        if ("service".equals(tagName)) {
                            ...
                            StApduServiceInfo serviceInfo = serviceCache.services.get(
                                    currComponent);
        ...
        try {
            ...
            for (int userId = 0; userId < mUserServices.size(); userId++) {
                ...
                for (StApduServiceInfo serviceInfo : userServices.services.values()) {
                    ...
                    Log.d(TAG, "writeServiceStateToFile() - component name " + serviceInfo.getComponent().flattenToString());
                    ...
                    if (installedServices.containsKey(Integer.toString(serviceInfo.getUid()))) {
                        HashMap<ComponentName, Integer> componentStates = installedServices.get(
                                Integer.toString(serviceInfo.getUid()));
    ...
    public int updateServiceState(int userId, int uid,
        ...
        synchronized (mLock) {
            ...
            Log.d(TAG, "2" + Thread.currentThread().getStackTrace()[2].getMethodName()
                    + ":WriteServiceStateToFile");
    ...
    public boolean registerAidGroupForService(int userId, int uid,
            ComponentName componentName, StAidGroup stAidGroup) {
        ...
        synchronized (mLock) {
            ...
            StApduServiceInfo serviceInfo = getStService(userId, componentName);
            if (serviceInfo == null) {
                Log.e(TAG, "registerAidGroupForService() - Service " + componentName
                        + " does not exist.");
                return false;
            }
            if (serviceInfo.getUid() != uid) {
            ...
            if (success) {
                newServices =
                    new ArrayList<StApduServiceInfo>(services.services.values());
            } else {
            ...
            for (StApduServiceInfo adpuServiceInfo : newServices) {
                if (adpuServiceInfo.getComponent().equals(componentName)) {
                    ...
                    if (adpuServiceInfo.hasCategory(CardEmulation.CATEGORY_OTHER) == true) {
                        adpuServiceInfo.setServiceState(CardEmulation.CATEGORY_OTHER,
                                StConstants.SERVICE_STATE_ENABLED);
                    }
    ...
    public StAidGroup getAidGroupForService(int userId, int uid, ComponentName componentName,
            String category) {
    ...
    public boolean removeAidGroupForService(int userId, int uid, ComponentName componentName,
        ...
        synchronized (mLock) {
            ...
            if (serviceInfo != null) {
                ...
                Log.e(TAG, "removeAidGroupForService() - Service " + componentName
                        + " does not exist.");
            }
        }
        if (success) {
    ...
    public void updateStatusOfServices(boolean commitStatus) {
        ...
        Log.d(TAG, "updateStatusOfServices() - 3 "
                + Thread.currentThread().getStackTrace()[2].getMethodName()
                + ": WriteServiceStateToFile");
