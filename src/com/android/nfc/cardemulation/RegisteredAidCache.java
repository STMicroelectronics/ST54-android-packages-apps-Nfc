...
public class RegisteredAidCache {
        ...
        for (ServiceAidInfo serviceAidInfo : conflictingServices) {
            ...
            if (serviceAidInfo.service.getComponent().equals(mPreferredForegroundService)) {
                ...
                if (serviceClaimsPaymentAid) {
                    ...
                    // but not as default.
                    if (DBG) Log.d(TAG, "resolveAidConflictLocked() - (Ignoring handling service " +
        ...
        if (aidDefaultInfo.foregroundDefault != null) {
            ...
            if (isSubset(aidServices.get(0).aid)) {
                ...
                resolveinfo.prefixInfo = findPrefixConflictForSubsetAid(aidServices.get(0).aid ,
                        new ArrayList<StApduServiceInfo>(){{add(resolveinfo.defaultService);}},true);
            }
             return resolveinfo;
            ...
            if (conflictingDefaultInfo.foregroundDefault != null ||
                ...
                if (isSubset(aidServices.get(0).aid)) {
                    ArrayList <StApduServiceInfo> apduServiceList = new  ArrayList <StApduServiceInfo>();
    ...
    void generateServiceMapLocked(List<StApduServiceInfo> services) {
        ...
        for (StApduServiceInfo service : services) {
            if (DBG) Log.d(TAG,
                    "generateServiceMapLocked() - component: " + service.getComponent());
            ...
            for (String aid : service.getAids()) {
                ...
                if (!CardEmulation.isValidAid(aid)) {
                    if(DBG2) Log.e(TAG, "generateServiceMapLocked() - Aid " + aid + " is not valid.");
                    continue;
                }
                if (aid.endsWith("*") && !supportsAidPrefixRegistration()) {
                    if(DBG2) Log.e(TAG, "generateServiceMapLocked() - Prefix AID " + aid
                    ...
                    for (String prefixAid : prefixAids) {
                        ...
                        if (aid.startsWith(prefix)) {
                            if(DBG2) Log.e(TAG, "generateServiceMapLocked() - Ignoring exact AID " + aid
                    ...
                    if(DBG2) Log.e(TAG, "Subset AID " + aid + " ignored on device that doesn't support it.");
                    continue;
                    ...
                    for (String subsetAid : subSetAids) {
                        ...
                        if (plainSubset.startsWith(aid)) {
                            if(DBG2) Log.e(TAG, "Ignoring exact AID " + aid + " because subset AID " + plainSubset +
                                    " is already registered");
                            foundSubset = true;
                            break;
                        }
    ...
    ReslovedPrefixConflictAid findPrefixConflictForSubsetAid(String subsetAid,
            ArrayList<StApduServiceInfo> prefixServices, boolean priorityRootAid) {
    ...
    void generateAidCacheLocked() {
        ...
        aidCache.clear();
        if (DBG) Log.d(TAG,
                "generateAidCacheLocked() - Nb of AIDs to process: " + aidsToResolve.size());
                    ...
                    for (String aid : resolveInfo.defaultService.getSubsetAids()) {
                        if (prefixConflicts.aids.contains(aid)) {
                            if ((CardEmulation.CATEGORY_PAYMENT.equals(resolveInfo.defaultService.getCategoryForAid(aid))) ||
                                ...
                                if(DBG2) Log.d(TAG, "AID " + aid+ " shared with prefix; " +
                                                "adding subset .");
                             }
                        }
                   }
...
//                if (DBG) Log.d(TAG, "Exact AID, resolving.");
    ...
    void updateRoutingLocked(boolean force) {
        ...
        for (Map.Entry<String, AidResolveInfo> aidEntry:
            ...
            if (!resolveInfo.mustRoute) {
                if (DBG) Log.d(TAG,
                        "updateRoutingLocked() - Not routing AID " + aid + " on request.");
                continue;
            }
    ...
    public void onPreferredPaymentServiceChanged(ComponentName service) {
        if (service != null) {
            if (DBG) Log.d(TAG,
                    "onPreferredPaymentServiceChanged() - service: " + service.flattenToString());
    ...
    public void onPreferredForegroundServiceChanged(ComponentName service) {
        ...
        if (service != null) {
            if (DBG) Log.d(TAG, "onPreferredForegroundServiceChanged() - service: "
    ...
    String dumpEntry(Map.Entry<String, AidResolveInfo> entry) {
        ...
        sb.append(
                "HCENfc_RegisteredAidCache -     \"" + entry.getKey() + "\" (category: " + category
                        + ")\n");
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println("HCENfc_RegisteredAidCache -     Service preferred by foreground app: "
                + mPreferredForegroundService);
        pw.println("HCENfc_RegisteredAidCache -     Preferred payment service: "
