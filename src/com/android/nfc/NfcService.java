...
public class NfcService implements DeviceHostListener {
    ...
    // static final boolean DBG2 = DBG && android.os.Build.IS_DEBUGGABLE;
    static final boolean DBG2 = true;
    ...
    private static final String PREF_LAST_MODIFIED_SERVICE =
    ...
    static final int MGS_UPDATE_ROUTING_TABLE = 21;

    static final int MGS_START_POLLING = 22;
    ...
    static final int INIT_WATCHDOG_MS = 15000;
    ...
    static final String OVERFLOW_NOTIFICATION_CHANNEL =
    ...
    private String[] mSpecialReaderList =
      {"com.CitizenCard.lyg", "com.txmpay.csewallet "};

    public void enforceNfceeAdminPerm(String pkg) {

        if (DBG2)
            Log.d(TAG, "enforceNfceeAdminPerm() - pkg: " + pkg);
        ...
        if (!mNfceeAccessControl.check(Binder.getCallingUid(), pkg)) {
            if (DBG2)
                Log.e(TAG, "enforceNfceeAdminPerm() - no access ");
        ...
        if (UserHandle.getCallingUserId() != UserHandle.USER_OWNER) {
            if (DBG2)
                Log.e(TAG,
                        "enforceNfceeAdminPerm() - only the owner is allowed to call SE APIs");
            throw new SecurityException(
                    "only the owner is allowed to call SE APIs");
        }
    }
   ...
   */
    ...
    public void onEeUpdated() {
        ...
        if (this.mState != NfcAdapter.STATE_TURNING_ON) {
            ...
            sendMessage(NfcService.MGS_UPDATE_ROUTING_TABLE, null);
        } else {
            if (DBG2)
    ...
    @Override
    public void onDefaultRoutesSet(int aidRoute, int mifareRoute, int isoDepRoute,
        ...
        if (DBG2)
            Log.d(TAG, "onDefaultRoutesSet() - aidRoute: " + String.format("0x%02X", aidRoute) + "/" + mUsedAidRoute +
                    ", mifareRoute: " + String.format("0x%02X", mifareRoute) + "/" + mUsedMifareRoute +
                    ", isoDepRoute: " + String.format("0x%02X", isoDepRoute) + "/" + mUsedIsoDepRoute +
                    ", felicaRoute: " + String.format("0x%02X", felicaRoute) + "/" + mUsedFelicaRoute +
                    ", abTechRoute" + String.format("0x%02X", abTechRoute) + "/" + mUsedAbTechRoute +
                    ", scRoute" + String.format("0x%02X", scRoute) + "/" + mUsedScRoute);
    ...
    public void onESEUnrecoverableError(){
        ...
        sendMessage(NfcService.MSG_RESTART_NFC_SERVICE, null);
    }
    ...
    public NfcService(Application nfcApplication) {
        ...
        NotificationChannel notificationChannel =
                new NotificationChannel(OVERFLOW_NOTIFICATION_CHANNEL,
        ...
        ServiceManager.addService(NfcSettingsAdapter.SERVICE_SETTINGS_NAME,

                mNfcSettingsAdapterService);
				}
        ServiceManager.addService(NfcAdapterStExtensions.SERVICE_NAME,
                mStExtras);
    ...
    private boolean tryBindSecureElementService() {
        ...
        if (mSEService == null) {
            ...
            if (bindingSuccess) {
                ...
                if (comp == null) {
                    Log.e(TAG, "tryBindSecureElementService() - bindService failed, could not resolve " + startIntent.toString());
                } else {
    ...
    private ServiceConnection mSecureElementConnection = new ServiceConnection() {
    ...
    void updatePackageCache() {
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < packagesNfcEvents.size(); i++) {
                if (DBG2) Log.d(TAG, "updatePackageCache() -- added " + packagesNfcEvents.get(i).packageName);
    ...
    void tryWaitForSEService() {
        ...
        Log.d(TAG, "Waited for SE service: " + (30 - i) + " loops , result:"
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        protected Void doInBackground(Integer... params) {
            ...
            switch (params[0].intValue()) {
                case TASK_ENABLE:
                if (DBG2)
                    Log.d(TAG, "EnableDisableTask - TASK_ENABLE");

                    enableInternal();
                    break;
                case TASK_DISABLE:
                if (DBG2)
                    Log.d(TAG, "EnableDisableTask - TASK_DISABLE");
                    disableInternal();
                    break;
                case TASK_BOOT:
                    boolean initialized;
                ...
                if (DBG2)
                    Log.d(TAG, "EnableDisableTask - TASK_BOOT");
        ...
        boolean enableInternal() {
            if (DBG2)
                Log.d(TAG, "EnableDisableTask - enableInternal(begin)");
...
//            applyRouting(true);
            ...
            if (DBG2)
                Log.d(TAG, "EnableDisableTask - enableInternal(end)");
        ...
        boolean disableInternal() {
            ...
            if (DBG2)
                Log.d(TAG, "EnableDisableTask - disableInternal(begin)");
            ...
            if (DBG2)
                Log.d(TAG, "EnableDisableTask - disableInternal(end)");
    ...
    final class NfcSettingsAdapterService extends INfcSettingsAdapter.Stub {
        ...
        public int getModeFlag(int mode) {
            Log.d(TAG, "NfcSettingsAdapterService - getModeFlag() - mode: "
                    + mode);

            // NfcPermissions.enforceAdminPermissions(mContext);
            return NfcAddonWrapper.getInstance().getModeFlag(mode,
                    NfcService.this);
        }

        @Override
        public void setModeFlag(int mode, int flag) {
            Log.d(TAG, "NfcSettingsAdapterService - setModeFlag() - set mode: "
                    + mode + ", flag: " + flag);
            ...
            if (mode == NfcSettingsAdapter.MODE_HCE) {
                currentHceMode =
                        NfcAddonWrapper.getInstance().getModeFlag(
                                NfcSettingsAdapter.MODE_HCE, NfcService.this);
            }

            // NfcPermissions.enforceAdminPermissions(mContext);
            NfcAddonWrapper.getInstance().setModeFlag(isNfcEnabled(), mode,
                    flag, NfcService.this);

            // Update current params
            NfcDiscoveryParameters newParams =
                    computeDiscoveryParameters(mScreenState);
            if (!newParams.equals(mCurrentDiscoveryParameters)) {
                Log.d(TAG,
                        "NfcSettingsAdapterService - setModeFlag() - NewParamRF");
        ...
        public boolean isRoutingTableOverflow() {
            ...
            if (DBG2)
                Log.d(TAG,
                        "NfcSettingsAdapterService - isRoutingTableOverflow() - status: "
                                + status);
        ...
        public boolean isShowOverflowMenu() {
            ...
            if (status == false) {
                ...
                try {
                    services =
                            mCardEmulationManager.getStServices(getUserId(),
                ...
                for (StApduServiceInfo srv : services) {
                    int state =
                            srv.getServiceState(CardEmulation.CATEGORY_OTHER);
            ...
            if (DBG2)
                Log.d(TAG,
                        "NfcSettingsAdapterService - isShowOverflowMenu() - status: "
                                + status);
        ...
        public List<ServiceEntry> getServiceEntryList(int userHandle) {
            if (DBG2)
                Log.d(TAG, "NfcSettingsAdapterService - getServiceEntryList()");
            NfcPermissions.enforceAdminPermissionsClient(mContext);
            ...
            final ArrayList<ServiceEntry> result =
            ...
            try {
                services =
                        mCardEmulationManager.getStServices(userHandle,
                                CardEmulation.CATEGORY_OTHER);
            } catch (RemoteException e) {
                Log.e(TAG, "Remote binder has already died.");
            ...
            for (StApduServiceInfo srv : services) {
                ...
                Integer sizeInLmrt =
                        new Integer(srv.getCatOthersAidSizeInLmrt());
                ...
                ServiceEntry srvEntry =
                        new ServiceEntry(comp, tag, title, banner, wasEnabled,
                                wantEnabled);
        ...
        public boolean testServiceEntryList(List<ServiceEntry> proposal) {
            boolean result;
            if (DBG2)
                Log.d(TAG, "NfcSettingsAdapterService - testServiceEntryList()");
            NfcPermissions.enforceAdminPermissionsClient(mContext);
        ...
        public void commitServiceEntryList(List<ServiceEntry> proposal) {
            if (DBG2)
                Log.d(TAG,
                        "NfcSettingsAdapterService - commitServiceEntryList()");
            NfcPermissions.enforceAdminPermissionsClient(mContext);
            Map<String, Boolean> serviceStates = new HashMap<String, Boolean>();
            for (ServiceEntry entry : proposal) {
                serviceStates.put(entry.getComponent().flattenToString(), entry
                        .getWantEnabled().booleanValue());
            }
            mCardEmulationManager.updateServiceState(UserHandle.myUserId(),
                    serviceStates);
        }

        @Override
        public List<ServiceEntry>
        getNonAidBasedServiceEntryList(int userHandle) {
            // if (DBG2) Log.d(TAG,
        ...
        public boolean EnableSE(String se_id, boolean enable) {
            ...
            return NfcAddonWrapper.getInstance().EnableSecureElement(se_id,
                    enable);
        }
        ...
        public void registerNfcSettingsCallback(INfcSettingsCallback cb) {
            NfcPermissions.enforceAdminPermissionsClient(mContext);
            if (cb != null)
                NfcAddonWrapper.getInstance().registerNfcSettingsCallback(cb);
        }
        ...
        public void unregisterNfcSettingsCallback(INfcSettingsCallback cb) {
            NfcPermissions.enforceAdminPermissionsClient(mContext);
            if (cb != null)
                NfcAddonWrapper.getInstance().unregisterNfcSettingsCallback(cb);
        }
        ...
        public void setDefaultUserRoutes(List<DefaultRouteEntry> userRoutes){
            ...
            for(DefaultRouteEntry entry : userRoutes){
                ...
                switch (routeName) {
                    ...
                    case NfcSettingsAdapter.DEFAULT_MIFARE_ROUTE:
                        if (mPrefs.getString(PREF_DEFAULT_MIFARE_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE) != routeLoc) {
                            mPrefsEditor = mPrefs.edit();
                    ...
                    case NfcSettingsAdapter.DEFAULT_ISO_DEP_ROUTE:
                        if (mPrefs.getString(PREF_DEFAULT_ISODEP_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE) != routeLoc) {
                            mPrefsEditor = mPrefs.edit();
                    ...
                    case NfcSettingsAdapter.DEFAULT_FELICA_ROUTE:
                        if (mPrefs.getString(PREF_DEFAULT_FELICA_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE) != routeLoc) {
                            mPrefsEditor = mPrefs.edit();
                    ...
                    case NfcSettingsAdapter.DEFAULT_AB_TECH_ROUTE:
                        if (mPrefs.getString(PREF_DEFAULT_AB_TECH_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE) != routeLoc) {
                            mPrefsEditor = mPrefs.edit();
                    ...
                    case NfcSettingsAdapter.DEFAULT_SC_ROUTE:
                        if (mPrefs.getString(PREF_DEFAULT_SC_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE) != routeLoc) {
                            mPrefsEditor = mPrefs.edit();
            ...
            mDeviceHost.setUserDefaultRoutesPref(convertPrefRouteToNfceeIdType(PREF_DEFAULT_MIFARE_ROUTE),
                    convertPrefRouteToNfceeIdType(PREF_DEFAULT_ISODEP_ROUTE),
                    convertPrefRouteToNfceeIdType(PREF_DEFAULT_FELICA_ROUTE),
                    convertPrefRouteToNfceeIdType(PREF_DEFAULT_AB_TECH_ROUTE),
                    convertPrefRouteToNfceeIdType(PREF_DEFAULT_SC_ROUTE),
        ...
        public List<DefaultRouteEntry> getDefaultUserRoutes(){
            ...
            if (DBG2) {
                Log.d(TAG,
                        "StNfcAdapterService - getDefaultUserRoutes()");
            }

            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_AID_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_AID_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_MIFARE_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_MIFARE_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_ISO_DEP_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_ISODEP_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_FELICA_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_FELICA_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_AB_TECH_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_AB_TECH_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_SC_ROUTE,
                    mPrefs.getString(PREF_DEFAULT_SC_ROUTE, NfcSettingsAdapter.DEFAULT_ROUTE)));
        ...
        public List<DefaultRouteEntry> getEffectiveRoutes(){
            ...
            if (DBG2) {
                Log.d(TAG,
                        "StNfcAdapterService - getEffectiveRoutes()");
            }

            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_AID_ROUTE, mUsedAidRoute));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_MIFARE_ROUTE, mUsedMifareRoute));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_ISO_DEP_ROUTE, mUsedIsoDepRoute));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_FELICA_ROUTE, mUsedFelicaRoute));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_AB_TECH_ROUTE, mUsedAbTechRoute));
            userRoutes.add(new DefaultRouteEntry(NfcSettingsAdapter.DEFAULT_SC_ROUTE, mUsedScRoute));
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public boolean enable() throws RemoteException {
            if (DBG2)
                Log.d(TAG, "NfcAdapterService - enable()");
            NfcPermissions.enforceAdminPermissions(mContext);
        ...
        public boolean disable(boolean saveState) throws RemoteException {
            if (DBG2)
                Log.d(TAG, "NfcAdapterService - disable() - saveState:"
                        + saveState);
            NfcPermissions.enforceAdminPermissions(mContext);
        ...
        public boolean isNfcSecureEnabled() throws RemoteException {
            synchronized (NfcService.this) {
                if (DBG2)
                    Log.d(TAG, "NfcAdapterService - isNfcSecureEnabled() - "
                            + mIsSecureNfcEnabled);
        ...
        public boolean setNfcSecure(boolean enable) {
            if (DBG2)
                Log.d(TAG, "NfcAdapterService - setNfcSecure() - " + enable);
            NfcPermissions.enforceAdminPermissions(mContext);
        ...
        public void setReaderMode(IBinder binder, IAppCallback callback, int flags, Bundle extras)
                throws RemoteException {
            if (DBG2)
                Log.d(TAG, "setReaderMode() flags:" + flags);

            String appName[] = mContext.getPackageManager().getPackagesForUid(Binder.getCallingUid());

            for(String name : appName){
                if (DBG2)
                    Log.d(TAG, "setReaderMode() calling package: " + name);

                for(String specialApp : mSpecialReaderList){
                  if(name.contains(specialApp)){
                    if(flags != 0){
                    if (DBG2)
                        Log.d(TAG, "setReaderMode() - enable special MIFARE SAK handling mode ");
                    mDeviceHost.enableSkipMifareInterface(true);
                    } else {
                        if (DBG2)
                            Log.d(TAG, "setReaderMode() - disable special MIFARE SAK handling mode ");
        ...
        public INfcAdapterExtras getNfcAdapterExtrasInterface(String pkg)
                throws RemoteException {
            if (DBG2)
                Log.d(TAG, "getNfcAdapterExtrasInterface() pkg:" + pkg);
        ...
        public boolean deviceSupportsNfcSecure() {
            ...
            if (TextUtils.isEmpty(sku) || !ArrayUtils.contains(skuList, sku)) {
                if (DBG2)
                    Log.d(TAG,
                            "NfcAdapterService - deviceSupportsNfcSecure() - false");
                return false;
            }
            if (DBG2)
                Log.d(TAG,
                        "NfcAdapterService - deviceSupportsNfcSecure() - true");
            return true;
        }
    ...
    final class StExtrasService extends INfcAdapterStExtensions.Stub {
        ...
        public void setRfConfiguration(int modeBitmap, byte[] techArray) {
            Log.i(TAG,
                    "setRfConfiguration() -  modeBitmap"
                            + String.valueOf(modeBitmap));
            synchronized (NfcService.this) {
        ...
        public void setRfBitmap(int modeBitmap) {
            Log.i(TAG,
                    "setRfBitmap() - modeBitmap: "
                            + Integer.toHexString(modeBitmap));
            mStExtensions.setRfBitmap(modeBitmap);
        }

        @Override
        public boolean getProprietaryConfigSettings(int SubSetID, int byteNb,
                int bitNb) {
            boolean status =
                    mStExtensions.getProprietaryConfigSettings(SubSetID,
                            byteNb, bitNb);
            Log.i(TAG,
                    "getProprietaryConfigSettings() SubSet ID "
                            + Integer.toHexString(SubSetID) + "- status =  "
                            + status);
            return status;
        }

        @Override
        public void setProprietaryConfigSettings(int SubSetID, int byteNb,
                int bitNb, boolean status) {
            Log.i(TAG, "setProprietaryConfigSettings() - byteNb = " + byteNb
                    + " bitNb = " + bitNb + " status =  " + status);
            mStExtensions.setProprietaryConfigSettings(SubSetID, byteNb, bitNb,
                    status);
        }
        ...
        public void getPipeInfo(int hostId, int pipeId, byte[] info) {
            Log.i(TAG, "getPipeInfo() - hostId =  " + hostId + "pipeId = "
                    + pipeId);
        ...
        public boolean getBitPropConfig(int configId, int byteNb, int bitNb) {
            return mStExtensions.getProprietaryConfigSettings(configId, byteNb,
                    bitNb);
        }

        public void setBitPropConfig(int configId, int byteNb, int bitNb,
                boolean status) {
            mStExtensions.setProprietaryConfigSettings(configId, byteNb, bitNb,
                    status);
        }
        ...
        public void sendPropSetConfig(int subSetId, int configId, byte[] param) {
            mStExtensions.sendPropSetConfig(subSetId, configId, param,
                    param.length);
        ...
        public byte[] sendPropTestCmd(int subCode, byte[] paramTx) {
            return mStExtensions.sendPropTestCmd(subCode, paramTx,
                    paramTx.length);
        }
    ...
    final class StDtaExtrasService extends INfcAdapterStDtaExtensions.Stub {
        ...
        public void setConnectionDevicesLimit(byte bCdlA, byte bCdlB, byte bCdlF, byte bCdlV) {
            ...
            Log.i(TAG, "setConnectionDevicesLimit() typeA: " + bCdlA + ", typeB: " + bCdlB + ", typeF: " + bCdlF + ", typeV: " + bCdlV);
        ...
        public void setSnepMode(byte role, byte server_type, byte request_type, byte data_type, boolean disc_incorrect_len){
            ...
            Log.i(TAG, "setSnepMode() role: " + role + ", server_type: " + server_type + ", request_type: " + request_type +
            		", data_type: " + data_type + ", disc_incorrect_len: " + disc_incorrect_len);
            mStDtaExtensions.setSnepMode(role, server_type, request_type, data_type, disc_incorrect_len);
        }

        public int enableDiscovery(byte con_poll, byte con_listen_dep, byte con_listen_t4tp, boolean con_listen_t3tp, boolean con_listen_acm,
        		                   byte con_bitr_f, byte con_bitr_acm){

        	Log.i(TAG, "enableDiscovery() con_poll: " + con_poll + ", con_listen_dep: " + con_listen_dep +
        			   ", con_listen_t4tp: " + con_listen_t4tp + ", con_listen_t3tp: " + con_listen_t3tp + ", con_listen_acm: " + con_listen_acm +
        			   ", con_bitr_f: " + con_bitr_f + ", con_bitr_acm: " + con_bitr_acm);
        	int result = mStDtaExtensions.enableDiscovery(con_poll, con_listen_dep, con_listen_t4tp, con_listen_t3tp, con_listen_acm, con_bitr_f, con_bitr_acm);
			return result;
        }
    ...
    final class TagService extends INfcTag.Stub {
        ...
        public int connect(int nativeHandle, int technology) throws RemoteException {
            ...
            if (DBG2) {
                Log.d(TAG, "TagService - connect() -nativeHandle:"
                        + nativeHandle + " technology:" + technology);
            }
        ...
        public int reconnect(int nativeHandle) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - reconnect() - nativeHandle:"
                        + nativeHandle);
        ...
        public boolean isPresent(int nativeHandle) throws RemoteException {
            TagEndpoint tag = null;
            if (DBG2)
                Log.d(TAG, "TagService - isPresent() -nativeHandle:"
                        + nativeHandle);
        ...
        public boolean isNdef(int nativeHandle) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - isNdef() -nativeHandle:"
                        + nativeHandle);
        ...
        public TransceiveResult transceive(int nativeHandle, byte[] data, boolean raw)
            ...
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - transceive() - nativeHandle:"
                        + nativeHandle);
        ...
        public NdefMessage ndefRead(int nativeHandle) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - ndefRead() - nativeHandle:"
                        + nativeHandle);
        ...
        public int ndefWrite(int nativeHandle, NdefMessage msg) throws RemoteException {
            ...
            if (DBG2)
                Log.d(TAG, "TagService - ndefWrite() - msg:" + msg.toString());
        ...
        public int formatNdef(int nativeHandle, byte[] key) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - formatNdef() nativeHandle:"
                        + nativeHandle);
        ...
        public Tag rediscover(int nativeHandle) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - rediscover() - nativeHandle:"
                        + nativeHandle);
        ...
        public int setTimeout(int tech, int timeout) throws RemoteException {
            NfcPermissions.enforceUserPermissions(mContext);
            if (DBG2)
                Log.d(TAG, "TagService - setTimeout() - tech:" + tech
                        + " timeout:" + timeout);
    ...
    final class NfcAdapterExtrasService extends INfcAdapterExtras.Stub {
        ...
        public Bundle open(String pkg, IBinder b) throws RemoteException {
            if (DBG2)
                Log.d(TAG, "NfcAdapterExtrasService - open() ");
        ...
        private int _open(IBinder b) {
            if (DBG2)
                Log.d(TAG, "NfcAdapterExtrasService - _open() ");
            synchronized (NfcService.this) {
                if (!isNfcEnabled()) {
...
//                }
                ...
                // accessed
                // the secure element.
                for (String packageName : mContext.getPackageManager()
    ...
    private class OpenSecureElement implements IBinder.DeathRecipient {
        ...
        public String toString() {
            ...
            .append(Integer.toHexString(hashCode())).append("[pid=")
            .append(pid).append(" handle=").append(handle).append("]")
            .toString();
        }
    }
    ...
    void applyRouting(boolean force) {
        synchronized (this) {
            ...
            if (DBG2)
                Log.d(TAG, "applyRouting() - exit");
        }
    }

    private NfcDiscoveryParameters computeDiscoveryParameters(int screenState) {
        if (DBG2) {
            Log.d(TAG, "computeDiscoveryParameters() screenState:"
        ...
        //Adapt to current screen state/sub power state with CON_DISCOVERY_PARAM and set_power_sub_state
            ...
            if (DBG2) {
                Log.d(TAG,
                        "computeDiscoveryParameters() - setTechMask "
                                ...
                                + Integer
                                .toHexString(techArrayConfigSave[3]));// P2P
                // Poll
            }
            ...
            if (DBG2) {
                Log.d(TAG,
                        "computeDiscoveryParameters() - setTechMask "
                                ...
                                + Integer
                                .toHexString(techArrayConfigSave[0]));// Reader
            }
        ...
        case ScreenStateHelper.SCREEN_STATE_ON_UNLOCKED:
            ...
            if (mReaderModeParams != null) {
                ...
                if (DBG2) {
                    Log.d(TAG, "mReaderModeParams != null setTechMask:"
                            + techMask);
                }
        ...
        case ScreenStateHelper.SCREEN_STATE_ON_LOCKED:
            ...
            boolean isLockscreenPollingEnabled =
    ...
    public int getAidRoutingTableSize() {
        ...
        if (DBG2)
            Log.d(TAG, "getAidRoutingTableSize() - aidTableSize: "
                    + aidTableSize);
    ...
    public void commitRouting() {
        if(!isNfcEnabled()){
            if (DBG)
                Log.d(TAG,
                        "commitRouting() - " +
                        "Not committing now because NFC state not yet ON");
            return;
        }
    ...
    public void startPollingLoop(){
        mHandler.sendEmptyMessage(MGS_START_POLLING);
    }

    /**
     * Set the ApduServiceInfo description of the last modified service that
     * caused an update of the routing table
     */
    public void setLastModifiedService(String description) {
        if (DBG2)
            Log.d(TAG, "setLastModifiedService() -  description: "
                    + description);
    ...
    /**
     * Get the ApduServiceInfo description of the last modified service that
     * caused an update of the routing table
     */
    public String getLastModifiedService() {
        String description = mPrefs.getString(PREF_LAST_MODIFIED_SERVICE, "");
        if (DBG2)
            Log.d(TAG, "getLastModifiedService() -  description: "
                    + description);
        return description;
    }
    ...
    public void resetOverflowSwitchInformation() {
        ...
        if (DBG2)
            Log.d(TAG, "resetOverflowSwitchInformation() ");
    ...
    /**
     * This is executed when user press the Cancel button of the notification in
     * case of overflow.
     */
    static public class NotificationBroadcastReceiver extends BroadcastReceiver {
        private CardEmulationManager mCardEmulationManager;
        ...
        static public NotificationBroadcastReceiver getInstance() {
            if (mSingleton == null) {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (getInstance().mCardEmulationManager != null) {
                getInstance().mCardEmulationManager
                .overflowNotificationClosed(OVERFLOW_NTF_ACTION_HIDE
                        .equals(action));
            }
        }
    }
    ...
    /**
     * AID routing table is full for all routes either start forced rouitng mode
     * is this is due to foregournd app or inform user, APDU service sin
     * categroy "other" need to be disabled
     */
    public void notifyRoutingTableFull() {
        if (DBG2)
            Log.d(TAG, "notifyRoutingTableFull() - !!!!overflow occured!!!!");
        ...
        if (mIsForegroundAllowed == true) {
            ...
            String description =
                    mPrefs.getString(PREF_LAST_MODIFIED_SERVICE, "");
            String msg;
            ...
            if (description != null && !description.equals("")) {
                msg =
                        String.format(mContext
                                .getString(R.string.aid_routing_overflow),
                                description);
            } else {
            ...
            if (mNotificationBroadcastReceiver == null) {
                NotificationBroadcastReceiver.createSingleton(mContext,
                        mCardEmulationManager);
                mNotificationBroadcastReceiver =
                        NotificationBroadcastReceiver.getInstance();
            }

            Intent intentCancel =
                    new Intent(OVERFLOW_NTF_ACTION_HIDE, null, mContext,
            ...
            .setContentTitle(
                    ...
                    .addAction(actCancel).addAction(actChange).build();
    ...
    public boolean getAidRoutingTableStatus() {
        ...
        if (DBG2)
            Log.d(TAG, "getAidRoutingTableStatus() -  mIsRoutingTableFull: "
                    + mIsRoutingTableFull);
    ...
    /**
     * Reset flag indicating AID routin gtable is full This is done when the
     * service status is updated
     */
    public void resetAidRoutingTableFull() {

        if (DBG2)
            Log.d(TAG, "resetAidRoutingTableFull()");
    ...
    /**
     * Start forced routing mode: This happens when a foreground app has caused
     * the routing table to be full
     */
    public void startForcedRouting(int nfceeId) {

        if (DBG2)
            Log.d(TAG, "startForcedRouting() -  nfceeId: " + nfceeId);
        mDeviceHost.forceRouting(nfceeId);
    }

    /**
     * Stop forced routing mode: This happens when the foreground app that has
     * caused the routing table to be full is sent to background/closed
     */
    public void stopForcedRouting() {

        if (DBG2)
            Log.d(TAG, "stopForcedRouting()");
        mDeviceHost.stopforceRouting();
    }

    /**
     * udpates information about foreground app: set/reset flag for foregournd
     * app indicates if foreground app is on host or off host
     */
    public void setForegroundAllowed(boolean status, String hostId) {
        if (DBG2)
            Log.d(TAG, "setForegroundAllowed() -  status: " + status
                    + ", hostId: " + hostId);
        ...
        if (status == false) {
            ...
            if (mIsForcedRouting == true) {
                ...
                if (DBG2)
                    Log.d(TAG, "setForegroundAllowed() -  Stop forced routing");
    ...
    HashMap<Integer, Integer> mOverflowRouteSizes =
            new HashMap<Integer, Integer>();

    public void setRoutingTableSizeFull(int route, int size) {
        if (DBG2)
            Log.d(TAG, "setRoutingTableSizeFull() -  route: " + route
                    + ", size: " + size);
    ...
    public void setRoutingTableSizeNotFull(int size) {
        if (DBG2)
            Log.d(TAG, "setRoutingTableSizeNotFull() - size: " + size);
    ...
    public void setRoutingTableSizeNotFullAlt(int size) {
        if (DBG2)
            Log.d(TAG, "setRoutingTableSizeNotFullAlt() - size: " + size);
    ...
    public int getRoutingTableSizeForRoute(int route) {
        ...
        if (DBG2)
            Log.d(TAG, "getRoutingTableSizeForRoute() -  route: " + route
                    + ", size: " + size);
...
//            }
        ...
        if (DBG2)
            Log.d(TAG,
                    "convertPrefRouteToNfceeIdType() - route Id: " + route + ", prefRoute: " + prefRoute + ", route: "
    ...
    public int getDefaultIsoDepRoute() {
        if (DBG2)
            Log.d(TAG,"getDefaultIsoDepRoute()");
    ...
    public int getConnectedNfceeId(int id){
        ...
        if (DBG2)
            Log.d(TAG,
                    "getConnectedNfceeId() -  requested id: " + String.format("0x%02X", id) +
    ...
    final class NfcServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                if (DBG2)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_ROUTE_AID)");
                    int route = msg.arg1;
                ...
                if (DBG2)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_UNROUTE_AID)");
                    String aid = (String) msg.obj;
...
//                loadWalletConfigFromPref();
                        ...
                        if (1 == Settings.Global.getInt(
                                mContext.getContentResolver(),
                                "nfc_rf_field_active", -1)) {
...
//                    }
                ...
                case MGS_UPDATE_ROUTING_TABLE:
                    if (DBG2)
                        Log.d(TAG,
                                "NfcServiceHandler - handleMessage(MGS_UPDATE_ROUTING_TABLE)");
                    updateRoutingTable();
                    break;
            ...
            case MSG_CLEAR_ROUTING: {
                if (DBG2)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_CLEAR_ROUTING)");
            ...
            case MSG_RESTART_NFC_SERVICE:
                ...
                if (NfcAddonWrapper.getInstance().getModeFlag(
                        NfcSettingsAdapter.MODE_READER, NfcService.this) == 0) {
                    if (DBG)
                        Log.d(TAG,
                ...
                if (DBG)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_LLCP_LINK_ACTIVATION)");
                ...
                if (DBG)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_LLCP_LINK_DEACTIVATED)");
                ...
                case MSG_LLCP_LINK_FIRST_PACKET:
                    if (DBG) Log.d(TAG, "NfcServiceHandler - handleMessage(MSG_LLCP_LINK_FIRST_PACKET)");
                ...
                if (DBG)
                    Log.d(TAG,
                            "NfcServiceHandler - handleMessage(MSG_TOAST_DEBOUNCE_EVENT)");				
                    sToast_debounce = false;
                    break;
					
                case MGS_START_POLLING:
                    if (DBG)
                        Log.d(TAG,
                                "NfcServiceHandler - handleMessage(MGS_START_POLLING)");
                    applyRouting(true);
                    break;
        ...
        private void sendOffHostTransactionEvent(byte[] aid, byte[] data, byte[] readerByteArray) {
            ...
            if (DBG2)
                Log.d(TAG, "NfcServiceHandler - sendOffHostTransactionEvent()");
            ...
            try {
                ...
                if (nfcAccess == null) {
                    if (DBG2) Log.d(TAG, "NfcServiceHandler - sendOffHostTransactionEvent() - nfcAccess is null");
                    return;
                }
                if (DBG2) Log.d(TAG, "NfcServiceHandler - sendOffHostTransactionEvent() - nfcAccess.length " + nfcAccess.length);
                ...
                for (int i = 0; i < nfcAccess.length; i++) {
                    if (DBG2) Log.d(TAG, "NfcServiceHandler - sendOffHostTransactionEvent() - " + nfcAccess[i] + " <- " + mNfcEventInstalledPackages.get(i));
                    if (nfcAccess[i]) {
        ...
        private ArrayList<String> getSEAccessAllowedPackages() {
            ...
            if (DBG2)
                Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages()");
            ...
            if (readers == null || readers.length == 0) {
                if (DBG2) Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages() - no reader");
                return null;
            }
            ...
            for (String reader : readers) {
                try {
                    boolean[] accessList = mSEService.isNFCEventAllowed(reader, null,
                            mNfcEventInstalledPackages.toArray(installedPackages));
                    if (accessList == null) {
                        if (DBG2) Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages() - r:" + reader + " - accessList is null");
                        continue;
                    }
                    ...
                    for (int i = 0; i < accessList.length; i++) {
                        if (accessList[i]) {
                            if (DBG2) Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages() - r:" + reader + " - accessList[" + i + "] is true");
            ...
            if (nfcAccessFinal == null) {
                if (DBG2) Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages() - accessListFinal is null");
                return null;
            }
            ArrayList<String> packages = new ArrayList<String>();
            for (int i = 0; i < nfcAccessFinal.length; i++) {
                if (nfcAccessFinal[i]) {
                    if (DBG2) Log.d(TAG, "NfcServiceHandler - getSEAccessAllowedPackages() - auth: " + mNfcEventInstalledPackages.get(i));
        ...
        private void sendNfcEeAccessProtectedBroadcast(Intent intent) {
            ...
            if (DBG2)
                Log.d(TAG,
                        "NfcServiceHandler - sendNfcEeAccessProtectedBroadcast()");
        ...
        private ArrayList<String> getNfcPreferredPaymentChangedSEAccessAllowedPackages() {
            ...
            if (DBG2)
                Log.d(TAG, "NfcServiceHandler - getNfcPreferredPaymentChangedSEAccessAllowedPackages()");
		
            if (!tryBindSecureElementService() || mNfcPreferredPaymentChangedInstalledPackages.isEmpty()) {
                return null;
            }
            String[] readers = null;
            try {
        ...
        private void sendPreferredPaymentChangedEvent(Intent intent) {
            ...
            if (DBG2)
                Log.d(TAG, "NfcServiceHandler - sendPreferredPaymentChangedEvent()");
            ...
            if (readerParams != null) {
                try {
                    ...
                    if (readerParams.callback != null) {
                        if (DBG2)
                            Log.d(TAG,
                                    "dispatchTagEndpoint() do onTagDiscovered()");
                        readerParams.callback.onTagDiscovered(tag);
                        if (DBG2)
                            Log.d(TAG, "onTagDiscovered() End");
                        return;
                    } else {
        ...
        private void applyScreenStateChange() {
            if (DBG)
                Log.d(TAG,
                        "NfcServiceHandler - applyScreenStateChange(MSG_APPLY_SCREEN_STATE)");
        ...
        private final ScheduledExecutorService mRFDBScheduler = Executors
        ...
        void debouncedRfField(int field) {
            ...
            if ((field == 0) && (mPendingRoutingTableUpdate == true)) {
                Log.d(TAG,
                        "debouncedRfField - applying postponed routing table update ");
            ...
            if ((field == 0) && (mPendingPowerStateUpdate == true)) {
                mScreenState = mScreenStateHelper.checkScreenState();
                Log.d(TAG,
                        "debouncedRfField - applying postponed screen state "
                                + mScreenState);
        ...
        private class RFDBRunnable implements Runnable {
            ...
            public void run() {
                synchronized (NfcService.this) {
                    if (DBG)
                        Log.d(TAG, "Sending RF_FIELD broadcast (debounced "
                                + mField + ")");
                    mRFDBScheduled = false;
                }
                ...
                if (DBG)
                    Log.d(TAG,
                            "NfcServiceHandler - debounceRfField() - debouncing RF_FIELD: "
                                    + field + " (cur:" + debouncedstate
                                    + ", pol:" + inversepolarity + ")");

                if (!isNfcEnabled()) {
                    if ((1 == Settings.Global.getInt(
                        mContext.getContentResolver(), "nfc_rf_field_active", -1)) && (field == 0)) {
                        Settings.Global.putInt(mContext.getContentResolver(), "nfc_rf_field_active", 0);
                        Intent fieldIntent = new Intent(ACTION_RF_FIELD_OFF_DETECTED);
                        sendNfcEeAccessProtectedBroadcast(fieldIntent);
                    }
                ...
                if (inversepolarity && (debouncedstate != 1) && (field == 1)) {
                    ...
                    if (mRFDBScheduled == false) {
                        ...
                        if (field == debouncedstate) {
                            ...
                            mRFDBScheduled = true;
                            mRFDBScheduledTask =
                                    mRFDBScheduler.schedule(sender, 150,
                ...
                if (DBG2) {
                    Log.d(TAG,
                            "ApplyRoutingTask.doInBackground() mScreenState:"
                                    + ScreenStateHelper
                                    .screenStateToString(mScreenState));
                }

                if (1 == Settings.Global.getInt(mContext.getContentResolver(), "nfc_rf_field_active", -1)) {
                ...
                if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                     if (mScreenState != ScreenStateHelper.SCREEN_STATE_OFF_LOCKED) {
                        ...
                        if (screenState == ScreenStateHelper.SCREEN_STATE_OFF_LOCKED) {
                            Log.d(TAG,
                                    "BroadcastReceiver - SCREEN_STATE_OFF_LOCKED");
                        } else if (screenState == ScreenStateHelper.SCREEN_STATE_OFF_UNLOCKED) {
                            Log.d(TAG,
                                    "BroadcastReceiver - SCREEN_STATE_OFF_UNLOCKED");
                        }
                    }
                    ...
                    if (screenState == ScreenStateHelper.SCREEN_STATE_ON_LOCKED) {
                        ...
                        Log.d(TAG,
                                "BroadcastReceiver - SCREEN_STATE_ON_UNLOCKED");
                    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_PACKAGE_REMOVED) ||
                ...
                if (isNfcEnabled()) {
                            ...
                            NfcAddonWrapper.getInstance().applyDeinitializeSequence(); // prevent SIM events
    ...
    /**
     * Update the status of all the services which were populated to commit to
     * routing table
     */
    public void updateStatusOfServices(boolean commitStatus) {

        if (DBG2)
            Log.d(TAG, "updateStatusOfServices() - commitStatus: "
                    + commitStatus);
