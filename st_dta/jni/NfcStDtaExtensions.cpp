...
bool NfcStDtaExtensions::initialize(bool nfc_state) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter; nfc_state=%d", 
	__func__, nfc_state);

    memset(&dta_info, 0, sizeof(tJNI_DTA_INFO));

    if (nfc_state == true) {
    	dta_info.dta_mode = DTA_MODE_OFF;
    }
    else {
...
void NfcStDtaExtensions::setPatternNb(uint32_t patternNb) {
  DLOG_IF(INFO, nfc_debug_enabled)
		<< StringPrintf("%s: enter; patternNb=0x%x", __func__, patternNb);
...
void NfcStDtaExtensions::setFsdFscExtension(uint32_t ext) {
    DLOG_IF(INFO, nfc_debug_enabled)
	 << StringPrintf("%s: enter; RF frame extension config=%d", __func__, ext);
